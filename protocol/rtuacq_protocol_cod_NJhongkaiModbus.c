...
int protocol_COD_NJhongkaiModbus(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, 0x01, 0x03, 0x09CB, 0x02);
	...
	SYSLOG_DBG("NJhongkai modbus COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJhongkai modbus COD data",com_rbuf,size);
