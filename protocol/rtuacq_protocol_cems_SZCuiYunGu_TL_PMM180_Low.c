...
Create Time:2021.09.25 Sun.
Description:protocol_CEMS_SZCuiYunGu_TL_PMM180_Low
...
static char SZCuiYunGu_TL_PMM180_Low_Mark = 'N';
...
#define POLCODE_NUM 1
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SZCuiYunGu TL_PMM180_Low CEMS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SZCuiYunGu TL_PMM180_Low CEMS protocol,CEMS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SZCuiYunGu TL_PMM180_Low CEMS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SZCuiYunGu TL_PMM180_Low CEMS RECV:",com_rbuf,size);
	...
	if(status == 0)
	{
		...
		acq_data->dev_stat = SZCuiYunGu_TL_PMM180_Low_Mark;
	}
	else 
...
Email:1148967988@qq.com
Create Time:2021.09.25 Sun.
Description:protocol_TSP_SZCuiYunGu_TL_PMM180_Low_info

TX1:01 03 00 64 00 03 44 14 
RX1:01 03 06 00 00 00 00 00 00 21 75

TX2:01 03 00 C8 00 28 C4 2A 
RX2:01 03 50 42 C8 00 00 20 20 10 16 08 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 
20 20 10 16 09 00 00 00 42 C8 00 00 3F 80 00 00 00 00 00 00 FF FF FF FF 
3F 00 00 00 FF FF FF FF FF FF FF FF FF FF FF FF 40 90 00 00 00 00 00 00 40 90 00 00 48 85

DataType and Analysis:
	(FLOAT_ABCD)   42 C8 00 00  = 100.00
*/

int protocol_TSP_SZCuiYunGu_TL_PMM180_Low_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"TL_PMM180_Low TSP INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("TL_PMM180_Low TSP INFO protocol,INFO 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("TL_PMM180_Low TSP INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"TL_PMM180_Low TSP INFO RECV1:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		...
		switch(val)
		{
			case 0: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,0,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'N';break;
			case 1: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,1,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'M';break;
			case 2: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,2,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'D';break;
			case 3: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,3,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'C'; break;
			case 4: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,5,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'z'; break;
			case 5: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,6,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'C';break;
			default: acqdata_set_value_flag(acq_data,"i12004",UNIT_NONE,99,INFOR_STATUS,&arg_n); SZCuiYunGu_TL_PMM180_Low_Mark = 'N'; break;
		}
		...
		switch(val)
		{
			...
			case 1: acqdata_set_value_flag(acq_data,"i12006",UNIT_NONE,1,INFOR_STATUS,&arg_n); break;
			case 2: acqdata_set_value_flag(acq_data,"i12006",UNIT_NONE,2,INFOR_STATUS,&arg_n); break;
			case 3: acqdata_set_value_flag(acq_data,"i12006",UNIT_NONE,3,INFOR_STATUS,&arg_n); break;
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"TL_PMM180_Low TSP INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("TL_PMM180_Low TSP protocol,INFO 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("TL_PMM180_Low TSP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"TL_PMM180_Low TSP INFO RECV2:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		valf = getFloatValue(p, 3, dataType);
