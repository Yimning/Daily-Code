...
int protocol_VOCs_ChuangChi4(struct acquisition_data *acq_data)
{
#define POLCODE_NUM 8

	char *polcode[POLCODE_NUM]={
		"e30004","e30104","e70004","e70104",
		"g28004","g28204","g20104","g29004"
	};
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ChuangChi VOC SEND4:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ChuangChi VOC protocol,VOC 4: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ChuangChi VOC data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ChuangChi VOC RECV4:",com_rbuf,size);
