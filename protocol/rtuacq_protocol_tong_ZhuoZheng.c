...
int protocol_TONG_ZhuoZheng(struct acquisition_data *acq_data)
{
   ...
   sleep(2);
	...
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZhuoZheng TONG protocol,TONG : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	//SYSLOG_DBG("data:%s\n",com_rbuf);
...
*/
		...
		f.ch[3] = com_rbuf[3];
		f.ch[2] = com_rbuf[4];
	SYSLOG_DBG("TONG data:%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,\n",
	com_rbuf[0],com_rbuf[1],com_rbuf[2],com_rbuf[3],
	com_rbuf[4],com_rbuf[5],com_rbuf[6],com_rbuf[7],com_rbuf[8]);
		
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w20138",UNIT_MG_L,(float)(f.f),&arg_n);
    
    }
    else
    {
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w20138",UNIT_MG_L,(float)(0.0),&arg_n);
		status = 1;
    }
...
int protocol_TONG_ZhuoZheng2(struct acquisition_data *acq_data)
{
	...
	if( (size >= 15)&&(com_rbuf[0] == 0x01)&&(com_rbuf[1] == 0x03) )
	{
		...
		switch(val)
		{
			...
			case 1: acq_data->instrstat=11;break;
			case 2: acq_data->instrstat=2;break;
			case 3: acq_data->instrstat=3;break;
			case 4: acq_data->instrstat=5;break;
			case 5: acq_data->instrstat=6;break;
			case 6: acq_data->instrstat=6;break;
			case 7: acq_data->instrstat=6;break;
			case 8: acq_data->instrstat=7;break;
			case 9: acq_data->instrstat=7;break;
			case 10: acq_data->instrstat=7;break;
			case 11: acq_data->instrstat=8;break;
			case 12: acq_data->instrstat=4;break;
			case 13: acq_data->instrstat=12;break;
			case 14: acq_data->instrstat=10;break;
			case 15: acq_data->instrstat=9;break;
			case 16: acq_data->instrstat=13;break;
			default: acq_data->instrstat=14;break;
		}
	...
	acqdata_set_value(acq_data,"w20138",UNIT_MG_L,tong,&arg_n);
	if(status == 0)
	acq_data->acq_status = ACQ_OK;
	else 
	acq_data->acq_status = ACQ_ERR;
