...
#define Fire_POLCODE_NUM   11
   char polcodestr[][20]={"SCXBH",
                                                                                      "CSXTLJCLL","TXSSTXHYJYL","TLSSBGFSHJYL","TLSSGFSHSYL","TLSSXSTNJYPHZ",
...
;
   ...
   int log_len=0;
	...
	if(!acq_data) return -1;
	   ...
	   {
	      SYSLOG_DBG("LaJi_fire protocol modbusarg  polcode  %d, %s,%x,%d\n",i,
		  	    mpolcodearg[i].polcode,mpolcodearg[i].regaddr&0xffff,mpolcodearg[i].datatype);
	   }

	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf, devaddr, 0x03,0x00, 26);
	SYSLOG_DBG_HEX("LaJi_fire protocol send data",com_tbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"LaJi_fire SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	
      	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
   	SYSLOG_DBG("LaJi_fire protocol : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LaJi_fire protocol recv data",com_rbuf,size);

	for(log_len=size;log_len>=200;log_len-=200)
	{
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"LaJi_fire modbus RECV:",&com_rbuf[size-log_len],log_len+1);
	}
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"LaJi_fire modbus RECV:",&com_rbuf[size-log_len],log_len+1);
			  ...
			  mpolcodearg_temp=find_modbus_polcode_arg_by_polcode(mpolcodearg,modbusarg_running->array_count,polcodestr[i]);
			   	...
			   	mpolcodearg_temp->enableFlag==1)
			   {
							...
							!strcmp(polcodestr[i],"GXSJ"))
					{
					   ...
					   SYSLOG_DBG("LaJi_fire protocol modbus_get_valf %s ,%f,%d,%d,tm=%04d%02d%02d%02d%02d%02d,tmlong=%d\n",polcodestr[i],valf,
...
;
   ...
   int log_len=0;
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf, devaddr, 0x03,modbusarg_running->devstaddr&0xffff, 26);
	SYSLOG_DBG_HEX("LaJi_fire protocol send data",com_tbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"LaJi_fire SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
