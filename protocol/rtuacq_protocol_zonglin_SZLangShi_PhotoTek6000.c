...
int protocol_ZONGLIN_SZLangShi_PhotoTek6000(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SZLangShi PhotoTek6000 ZONGLIN SEND:",com_tbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SZLangShi PhotoTek6000 ZONGLIN SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SZLangShi PhotoTek6000  ZONGLIN protocol,ZONGLIN1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SZLangShi PhotoTek6000  ZONGLIN data1",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SZLangShi PhotoTek6000  ZONGLIN RECV1:",com_rbuf,size);
	if((size>=11)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==cmd))
	{
		ZONGLIN_org = getFloatValue(com_rbuf, 3, dataType);
		val=com_rbuf[7];
		val<<=8;
		val+=com_rbuf[8];
		switch(val)
...
Author:Yimning
Time:2021.08.17 Tues.
Description:protocol_ZONGLIN_SZLangShi_PhotoTek6000_info
...
*/
int protocol_ZONGLIN_SZLangShi_PhotoTek6000_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	cmd = 0x03;
	dataType = FLOAT_CDAB;
	SYSLOG_DBG("protocol ZONGLIN SZLangShi PhotoTek6000  info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ZONGLIN SZLangShi PhotoTek6000  info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ZONGLIN SZLangShi PhotoTek6000 , Info : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ZONGLIN SZLangShi PhotoTek6000  Info data1",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ZONGLIN SZLangShi PhotoTek6000  info RECV1:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ZONGLIN SZLangShi PhotoTek6000  info SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ZONGLIN SZLangShi PhotoTek6000 , Info2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ZONGLIN SZLangShi PhotoTek6000  Info2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ZONGLIN SZLangShi PhotoTek6000  info RECV2:",com_rbuf,size);
	if((size>=13)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==cmd))
	{
		val = getInt16Value(com_rbuf, 3, INT_AB);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ZONGLIN SZLangShi PhotoTek6000  info SEND3:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ZONGLIN SZLangShi PhotoTek6000 , Info3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ZONGLIN SZLangShi PhotoTek6000  Info3 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ZONGLIN SZLangShi PhotoTek6000  info RECV3:",com_rbuf,size);
