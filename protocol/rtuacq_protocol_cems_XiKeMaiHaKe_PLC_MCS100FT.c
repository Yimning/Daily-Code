...
TX:01 03 00 96 00 04 A4 25
RX:01 03 08 16 23 16 24 16 25 82 7C
...
#define MODBUS_POLCODE_NUM 4
	...
	if(p!=NULL)
	{
		for(i=0;i<MODBUS_POLCODE_NUM;i++)
		{
			...
			acqdata_set_value(acq_data,polcode[i],polcodeUnit[i],valfArray[i],&arg_n);

			if(!strcmp(polcode[i],"a34013a"))
				acqdata_set_value(acq_data,"a34013",UNIT_MG_M3,valfArray[i],&arg_n);
	...
	else
	{
		for(i=0;i<MODBUS_POLCODE_NUM;i++)
		{
		       acqdata_set_value(acq_data,polcode[i],polcodeUnit[i],0,&arg_n);
			if(!strcmp(polcode[i],"a34013a"))
