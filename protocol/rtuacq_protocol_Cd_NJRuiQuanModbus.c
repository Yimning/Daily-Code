...
*/
	...
	SYSLOG_DBG("NJRuiQuan modbus Cd protocol,Cd : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJRuiQuan modbus Cd data ",com_rbuf,size);	
	...
	acqdata_set_value(acq_data,"w20115",UNIT_UG_L,Cd,&arg_n);
	if(status == 0)
