...
int protocol_CEMS_AHwanyi(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,0x2,0x3,0x04,0x16);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHwanyi CEMS protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("AHwanyi CEMS data",com_rbuf,size);

	len=ARRAY_SIZE(garbage_polcode);
	if((size>=49)&&(com_rbuf[0]==0x02)&&(com_rbuf[1]==0x03))
	{
		for(i=0;i<len;i++)
		{
