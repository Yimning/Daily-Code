...
Description:protocol_COD_HuiHuan
TX:%01#RDD0152001525**
...
Email:1148967988@qq.com
Create Time:2021.08.07 Sun.
Description:protocol_COD_HuiHuan_info
...
*/
protocol_COD_HuiHuan_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	char com_rbuf[2048]={0}; 
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND1:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV1:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND2:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV2:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND3:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV3:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND4:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO4 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV4:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND5:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO5 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV5:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND6:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO6 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV6:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND7:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO7 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV7:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan COD INFO SEND8:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan COD protocol,INFO8 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan COD INFO RECV8:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=12)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
