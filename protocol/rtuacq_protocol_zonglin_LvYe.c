...
int protocol_ZONGLIN_LvYe(struct acquisition_data *acq_data)
{
/*
	...
	zonglin = 33 B3 80 42 = 64.35 mg/L
*/
	...
	SYSLOG_DBG("LvYe zonglin protocol,zonglin : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LvYe zonglin data",com_rbuf,size);
	if(size>=11)
	{
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("LvYe zonglin status protocol,zonglin status: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LvYe zonglin status data",com_rbuf,size);
	if(size>=2)
	{
		val=com_rbuf[0];
		switch(val)
