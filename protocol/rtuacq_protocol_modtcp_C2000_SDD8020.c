...
{//modbus tcp
    ...
    char modbus_data[]={0x0,0x1,0x0,0x0,0x0,0x6,0xff,0x2,0x0,0xc8,0x00,0x08};
    ...
    acq_ctrl=ACQ_CTRL(acq_data);
     ...
     {
	    ret=acqdata_set_value(acq_data,"N4013M1",UNIT_NONE,0,&arg_n);
     ...
     if(ret>=10)
     {
	   ...
	   acqdata_set_value(acq_data,"N4013M1",UNIT_NONE,(float)(modbus_cache[9]),&arg_n);
...
int protocol_modtcp_C2000_SDD8020_2(struct acquisition_data *acq_data)
{
    ...
    acq_ctrl=ACQ_CTRL(acq_data);
     ...
     if(acq_ctrl->link_stat != ACQ_LINK_CONNECTED)
     {
	    ret=acqdata_set_value(acq_data,"N4013M2",UNIT_NONE,0,&arg_n);
		acq_data->acq_status = ACQ_ERR;
		read_rtc_time(acq_data->acq_tm); 
		return arg_n;
     }
     ...
     if(ret>=10)
     {
	   ...
	   acqdata_set_value(acq_data,"N4013M2",UNIT_NONE,(float)(modbus_cache[9]),&arg_n);
