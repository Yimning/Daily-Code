...
int protocol_ANDAN_GLYunJing_HeBei(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei NH3N SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei NH3N protocol,NH3N : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei NH3N data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei NH3N RECV:",com_rbuf,size);
	...
	else
	{
		...
		LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei NH3N SEND again:",com_tbuf,size);
		size=write_device(DEV_NAME(acq_data),com_tbuf,size);
		sleep(1);
		memset(com_rbuf,0,sizeof(com_rbuf));
		size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
		SYSLOG_DBG("GLYunJing HeBei NH3N protocol,NH3N : read again device %s , size=%d\n",DEV_NAME(acq_data),size);
		SYSLOG_DBG_HEX("GLYunJing HeBei NH3N data",com_rbuf,size);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei NH3N RECV again:",com_rbuf,size);
		p=modbus_crc_check(com_rbuf, size, devaddr, 0x03, 0x02);
		if(p!=NULL)
		{
			f.ch[0]=p[4];
...
static float  ANDAN_GLYunJing_HeBei_Stay = 0;
	int status=0;
	float *pf;
	union uf  f;
	char com_rbuf[2048]={0}; 
	...
	if(p!=NULL)
	{
		...
		switch(val)
		{
			case 0:	flag='N';ANDAN_GLYunJing_HeBei_Stay = valf;break;
			//case 1:	flag='T';break;
			//case 2:	flag='T';break;
			case 3:	flag='D';break;
			case 4:	flag='M';break;
...
int protocol_ANDAN_GLYunJing_HeBei_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ANDAN_GLYunJing_HeBei_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei NH3N STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei NH3N STATUS protocol,STATUS: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei NH3N STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei NH3N STATUS RECV:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei NH3N INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei NH3N INFO protocol,INFO 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei NH3N INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei NH3N INFO RECV1:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei NH3N INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei NH3N INFO protocol,INFO 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei NH3N INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei NH3N INFO RECV2:",com_rbuf,size);
