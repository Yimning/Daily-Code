...
int protocol_VOCs_ChuangChi1(struct acquisition_data *acq_data)
{
#define POLCODE_NUM 8

	char *polcode[POLCODE_NUM]={
		"e30001","e30101","e70001","e70101",
		"g28001","g28201","g20101","g29001"
	};
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ChuangChi VOC SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ChuangChi VOC protocol,VOC 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ChuangChi VOC data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ChuangChi VOC RECV1:",com_rbuf,size);
