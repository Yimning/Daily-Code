...
int protocol_CEMS_SHHeRu_APxx370(struct acquisition_data *acq_data)
{
	...
	if(isPolcodeEnable(modbusarg, "a21026"))
	{
		...
		size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "SHHeRu APSA370 CEMS", 1);
	...
	*/
	...
	if(isPolcodeEnable(modbusarg, "a21002"))
	{
		...
		size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "SHHeRu APNA370 CEMS", 1);
		p=modbus_crc_check(com_rbuf, size, devaddr, cmd, cnt);
		if(p!=NULL)
		{
...
int protocol_CEMS_SHHeRu_APxx370_SO2_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "SHHeRu APSA370 SO2 INFO", 1);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, cnt);
	if(p!=NULL)
	{
...
int protocol_CEMS_SHHeRu_APxx370_NOx_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "SHHeRu APNA370 NOx INFO", 1);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, cnt);
	if(p!=NULL)
	{
		valf=getUInt16Value(p, 3, INT_AB);
