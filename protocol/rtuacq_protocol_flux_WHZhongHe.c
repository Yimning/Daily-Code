...
int protocol_FLUX_WHZhongHe(struct acquisition_data *acq_data)
{
   ...
   union uf f;
	...
	size=modbus_pack(com_tbuf, 0x01, 0x03, 0x02, 0x06);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("flux WHZhongHe protocol : read device %s , size=%d\n",DEV_NAME(acq_data),size);
    SYSLOG_DBG_HEX("flux WHZhongHe protocol",com_rbuf,size);
