...
int protocol_FLUX_WoLun(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, devaddr, 0x03,0x0100 ,0x04);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"WoLun flux SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("WoLun flux protocol :flux read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("WoLun flux protocol",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"WoLun flux RECV:",com_rbuf,size);
