...
int protocol_ANDAN_ZhongLv(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv NH3N SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N RECV1:",com_rbuf,200);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N RECV2:",&com_rbuf[200],size-200);
...
int protocol_ANDAN_ZhongLv_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ANDAN_ZhongLv_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv NH3N STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N STATUS RECV:",com_rbuf,size);
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,0x3,0x02A4,0x6E);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv NH3N INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	if(size>200)
	{
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N INFO RECV1:",com_rbuf,200);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N INFO RECV2:",&com_rbuf[200],size-200);
	}
	else
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv NH3N INFO RECV:",com_rbuf,size);
	if((size>=225)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
