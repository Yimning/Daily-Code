...
int protocol_COD_JuGuangKeJi(struct acquisition_data *acq_data)
{
   ...
   union uf pf;
	...
	size=modbus_pack(com_tbuf, 0x30, 0x03, 0x04, 0x02);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf)-1);
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JuGuangKeJi COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JuGuangKeJi COD data\n",com_rbuf,size);
	
	if((size>=8)&&(com_rbuf[0]==0x30)&&(com_rbuf[1]==0x03))
		{
			pf.ch[0]=com_rbuf[6];
			pf.ch[1]=com_rbuf[5];
			pf.ch[2]=com_rbuf[4];
