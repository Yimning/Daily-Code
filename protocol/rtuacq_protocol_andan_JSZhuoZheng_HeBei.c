...
Author:Yimning
Time:2021.08.02 Mon.
Description:protocol_ANDAN_JSZhuoZheng_HeBei
...
int protocol_ANDAN_JSZhuoZheng_HeBei(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSZhuoZheng_HeBei ANDAN SEND:",com_tbuf,size);
...
Author:Yimning
Time:2021.08.02 Mon.
Description:protocol_ANDAN_JSZhuoZheng_HeBei_info
...
*/
int protocol_ANDAN_JSZhuoZheng_HeBei_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	float ANDAN_JSZhuoZheng_HeBei=0;
	...
	cmd = 0x03;
	dataType = FLOAT_ABCD;
	SYSLOG_DBG("protocol ANDAN JSZhuoZheng_HeBei info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ANDAN JSZhuoZheng_HeBei info SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ANDAN JSZhuoZheng_HeBei, Info : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ANDAN JSZhuoZheng_HeBei Info data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ANDAN JSZhuoZheng_HeBei info RECV:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ANDAN JSZhuoZheng_HeBei info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ANDAN JSZhuoZheng_HeBei, Info1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ANDAN JSZhuoZheng_HeBei Info1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ANDAN JSZhuoZheng_HeBei info RECV1:",com_rbuf,size);
