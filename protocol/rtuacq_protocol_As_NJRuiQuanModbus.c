...
*/
	...
	SYSLOG_DBG("NJRuiQuan modbus As protocol,As : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJRuiQuan modbus As data ",com_rbuf,size);	
	...
	acqdata_set_value(acq_data,"w20119",UNIT_UG_L,As,&arg_n);
	if(status == 0)
