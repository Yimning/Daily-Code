...
*/
int protocol_ANDAN_NJhongkai(struct acquisition_data *acq_data)
{
	int status=0;
   float *pf;
...
*/
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"NJHongKai ANDAN SEND:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJhongkai ANDAN protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("data:%s\n",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"NJHongKai ANDAN RECV:",com_rbuf,size);
