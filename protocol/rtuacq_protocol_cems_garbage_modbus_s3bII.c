...
static const  char * const garbage_polcode[] = {"i33310","i33311","i33312","i33320","i33321","i33322","a01901","a01902"};
...
int protocol_CEMS_garbage_modbus_s3bII(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,0x1,0x3,0x3b,0x09);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("garbage CEMS protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("garbage CEMS data",com_rbuf,size);
	...
	if((size>=23)&&(com_rbuf[0]==0x01)&&(com_rbuf[1]==0x03))
	{
		...
		acqdata_set_value(acq_data,garbage_polcode[7],UNIT_0C,temp,&arg_n);
