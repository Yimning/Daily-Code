...
int protocol_FLUX_HBKeruida(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, 0x01, 0x03, 0x04, 0x04);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("FLUX_HBKeruida protocol : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("FLUX_HBKeruida protocol",com_rbuf,size);
