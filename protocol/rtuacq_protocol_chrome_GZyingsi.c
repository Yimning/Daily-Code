...
int protocol_CHROME_GZyingsi(struct acquisition_data *acq_data)
{
   ...
   if(!acq_data) return -1;
	...
	strcpy(com_tbuf,"%03#RDD0048100484**");
   ...
   sleep(2);
	...
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("GZyingsi CHROME protocol,CHROME : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("data:%s\n",com_rbuf);
    
	pf=acq_data->com_f;       
   if( (size >= 24)&&(strncmp(com_rbuf,"%03$RD",6)==0))
   {               
		...
		pf[arg_n] /= 1000.0;
		strcpy(acq_data->polcode[arg_n],"023"); 
