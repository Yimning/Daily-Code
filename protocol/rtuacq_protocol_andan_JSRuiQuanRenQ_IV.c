...
Description:protocol_ANDAN_JSRuiQuanRenQ_IV
...
int protocol_ANDAN_JSRuiQuanRenQ_IV(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV ANDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV ANDAN protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV ANDAN data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV ANDAN RECV:",com_rbuf,size);
...
int protocol_ANDAN_JSRuiQuanRenQ_IV_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ANDAN_JSRuiQuanRenQ_IV_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV ANDAN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV ANDAN INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV ANDAN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV ANDAN INFO RECV:",com_rbuf,size);
