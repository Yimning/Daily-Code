...
Description:protocol_ZONGLIN_ZJHZLiQi_V2
...
int protocol_ZONGLIN_ZJHZLiQi_V2 (struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ZJHZLiQi V2 ZONGLIN SEND :",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZJHZLiQi V2 ZONGLIN protocol,ZONGLIN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZJHZLiQi V2 ZONGLIN  data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ZJHZLiQi V2 ZONGLIN RECV:",com_rbuf,size);
	...
	acqdata_set_value(acq_data,"w21011",UNIT_MG_L,valf,&arg_n);
...
int protocol_ZONGLIN_ZJHZLiQi_V2_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ZJHZLiQi V2 ZONGLIN INFO SEND1 :",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZJHZLiQi V2 ZONGDAN INFO protocol,ZONGLIN INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZJHZLiQi V2 ZONGLIN INFO1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ZJHZLiQi V2 ZONGLIN INFO RECV1:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ZJHZLiQi V2 ZONGLIN INFO SEND2 :",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZJHZLiQi V2 ZONGLIN INFO protocol,ZONGLIN INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZJHZLiQi V2 ZONGLIN INFO2  data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ZJHZLiQi V2 ZONGLIN INFO RECV2:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		valf = getFloatValue(p, 3, dataType);
