...
Description:protocol_FLUX_CangZhouTianYi
TX:01 03 05 4D 00 06 55 13
RX:01 03 0C 41 43 00 00 41 43 00 00 41 43 00 00 67 D9
...
int protocol_FLUX_CangZhouTianYi(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"CangZhouTianYi flux SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("CangZhouTianYi flux protocol :flux read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("CangZhouTianYi flux protocol",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"CangZhouTianYi flux RECV:",com_rbuf,size);
