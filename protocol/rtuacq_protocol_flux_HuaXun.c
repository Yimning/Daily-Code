...
int protocol_FLUX_HuaXun(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, devaddr, 0x03,0x0252 ,0x06);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HuaXun flux SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuaXun flux protocol :flux read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HuaXun flux protocol",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HuaXun flux RECV:",com_rbuf,size);
	...
	acqdata_set_value(acq_data,"w60103",UNIT_L_S,speed,&arg_n);	
	acqdata_set_value(acq_data,"w60003",UNIT_M3,total_flux_M3,&arg_n);
	...
	acqdata_set_value(acq_data,"wb0104",UNIT_L_S,speed,&arg_n);	
	acqdata_set_value(acq_data,"wb0004",UNIT_M3,total_flux_M3,&arg_n);
