...
int protocol_TSP_BJXueDiLong_Model2030(struct acquisition_data *acq_data) {
	...
	size=10;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"BJXueDiLong Model2030 TSP SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("BJXueDiLong Model2030 TSP protocol,TSP : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("BJXueDiLong Model2030 TSP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"BJXueDiLong Model2030 TSP RECV:",com_rbuf,size);
...
int protocol_TSP_BJXueDiLong_Model2030_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_TSP_BJXueDiLong Model2030_info\n");
	...
	size=10;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"BJXueDiLong Model2030 TSP STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("BJXueDiLong Model2030 TSP STATUS protocol,STATUS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("BJXueDiLong Model2030 TSP STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"BJXueDiLong Model2030 TSP STATUS RECV:",com_rbuf,size);
	if((size>=36)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x66)&&(com_rbuf[2]==0x1E))
	{
	...
	size=6;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"BJXueDiLong Model2030 TSP INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("BJXueDiLong Model2030 TSP protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("BJXueDiLong Model2030 TSP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"BJXueDiLong Model2030 TSP INFO RECV:",com_rbuf,size);
