...
Description:protocol_ZONGDAN_BJLiDa
...
int protocol_ZONGDAN_BJLiDa(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"BJLiDa HeBei ZONGDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("BJLiDa HeBei ZONGDAN protocol,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("BJLiDa HeBei ZONGDAN data",com_rbuf,size);
	...
	ret=acqdata_set_value(acq_data,"w21001",UNIT_MG_L,nh3n,&arg_n);
	if(status == 0)
...
int protocol_ZONGDAN_BJLiDa_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_BJLiDa_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd ,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"BJLiDa HeBei ZONGDAN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("BJLiDa HeBei ZONGDAN protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("BJLiDa HeBei INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"BJLiDa HeBei ZONGDAN INFO RECV:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p !=NULL)
	{
