...
Description:protocol_COD_JSRuiQuanRenQ_IV
...
int protocol_COD_JSRuiQuanRenQ_IV(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV COD protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV COD data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV COD RECV:",com_rbuf,size);
...
int protocol_COD_JSRuiQuanRenQ_IV_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_JSRuiQuanRenQ_IV_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV COD INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV COD INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV COD INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV COD INFO RECV:",com_rbuf,size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV COD data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV COD RECV:",com_rbuf,size);
