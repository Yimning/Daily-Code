...
{
/*
//miles zhang 20210710
RS485
TX:02 03 00 00 00 2c 44 24 
RX:
*/
#define POLCODE_NUM 6

	char *polcode[POLCODE_NUM]={
		"a19001a","a01012","a01013","a01011","a01014","a34013a"
	};

	UNIT_DECLARATION unit[POLCODE_NUM]={
		UNIT_PECENT,UNIT_0C,UNIT_PA,UNIT_M_S,UNIT_PECENT,UNIT_MG_M3
	};
	...
	int ret=0;
	int arg_n=0;
	int devaddr=2; //default devaddr = 2
	int i=0,j=0;
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SZCuiYunGu TL-PMM1804 CEMS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SZCuiYunGu TL-PMM1804 CEMS protocol,CEMS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SZCuiYunGu TL-PMM1804 CEMS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SZCuiYunGu TL-PMM1804 CEMS RECV:",com_rbuf,size);
	if((size>=88)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		for(i=2,j=0;i<(POLCODE_NUM+2);i++,j++)
		{
		    if(!strcmp(polcode[j],"a01013")) continue; //pass a01013
			...
			acqdata_set_value(acq_data,polcode[j],unit[j],f.f,&arg_n);
			if(!strcmp(polcode[j],"a34013a"))
			{
				acqdata_set_value(acq_data,"a34013",UNIT_MG_M3,f.f,&arg_n);
			}	
			if(!strcmp(polcode[j],"a19001a"))
			{
				acqdata_set_value(acq_data,"a19001",UNIT_PECENT,f.f,&arg_n);
			}				
		}

		f.ch[0]=com_rbuf[12*4+6];
		f.ch[1]=com_rbuf[12*4+5];
		f.ch[2]=com_rbuf[12*4+4];
		f.ch[3]=com_rbuf[12*4+3];
	...
	{
		for(j=0;j<POLCODE_NUM;j++)
		{
			acqdata_set_value(acq_data,polcode[j],unit[j],0,&arg_n);
			if(!strcmp(polcode[j],"a34013a"))
			{
				acqdata_set_value(acq_data,"a34013",UNIT_MG_M3,f.f,&arg_n);
			}	
...
/*
int protocol_CEMS_SZCuiYunGu_TL_PMM180_4_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
