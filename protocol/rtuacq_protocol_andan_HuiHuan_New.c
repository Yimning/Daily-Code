...
Description:protocol_ANDAN_HuiHuan_New
TX:%01#RDD0152001525**
...
int protocol_ANDAN_HuiHuan_New(struct acquisition_data *acq_data)
{
	int status=0;
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN SEND:",com_tbuf,size);
	size =write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN RECV:",com_rbuf,size);
	SYSLOG_DBG("HuiHuan ANDAN protocol,andan : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("HuiHuan ANDAN data: %s",com_rbuf);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=33)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&(check!=NULL))
	{
		strncpy(data_buff,&com_rbuf[22],8);
		tem=atoi(data_buff);
		andan=((float)tem)/1000.0;
...
Email:1148967988@qq.com
Create Time:2021.08.07 Sun.
Description:protocol_ANDAN_HuiHuan_New_info
...
*/
protocol_ANDAN_HuiHuan_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	char com_rbuf[2048]={0}; 
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND1:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV1:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND2:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV2:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND3:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV3:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND4:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO4 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV4:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND5:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO5 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV5:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND6:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO6 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV6:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND7:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO7 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV7:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ANDAN INFO SEND8:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ANDAN protocol,INFO8 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ANDAN INFO RECV8:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=12)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
