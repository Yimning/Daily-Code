...
int protocol_Lin_N_SHJingFang(struct acquisition_data *acq_data)
{
   ...
   union uf f;
	...
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
   	SYSLOG_DBG("SHJingFang ZONGLIN protocol,ZONGLIN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SHJingFang ZONGLIN data ",com_rbuf,size);
	...
	ret=acqdata_set_value(acq_data,"w21001",UNIT_MG_L,total_N,&arg_n);
	if(status == 0)
