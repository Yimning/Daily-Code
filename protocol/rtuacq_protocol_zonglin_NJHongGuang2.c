...
Description:protocol_ZONGLIN_NJHongGuang2
...
int protocol_ZONGLIN_NJHongGuang2_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
...
ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TP INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TP protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TP INFO RECV1:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, 0x3, 0x03);
	if(p!= NULL)
	{
...
ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TP INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TP protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TP INFO RECV2:",com_rbuf,size);
...
ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TP INFO SEND3:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(3);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TP protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TP INFO RECV3:",com_rbuf,size);
