...
{
/*
	TX:	07 03 00 00 00 02 ** **
	RX:	07 03 04 57 0A 42 95 FF FF

	cod = 57 0A 42 95 = 74.67
*/
	int status=0;
	float *pf;
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJRuiQuanModbus COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJRuiQuan modbus COD protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJRuiQuan modbus COD data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJRuiQuanModbus COD RECV:",com_rbuf,size);
...
int protocol_COD_NJRuiQuanModbus_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_NJRuiQuanModbus_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJRuiQuanModbus COD INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJRuiQuanModbus COD INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJRuiQuanModbus COD INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJRuiQuanModbus COD INFO RECV:",com_rbuf,size);
