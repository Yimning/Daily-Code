...
int protocol_ZONGNIE_ZhuoZheng(struct acquisition_data *acq_data)
{
   ...
   sleep(2);
	...
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZhuoZheng ZONGNIE protocol,ZONGNIE : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	//SYSLOG_DBG("data:%s\n",com_rbuf);
...
*/
	...
	com_rbuf[4],com_rbuf[5],com_rbuf[6],com_rbuf[7],com_rbuf[8]);
		...
		acqdata_set_value(acq_data,"w20121",UNIT_MG_L,(float)(f.f),&arg_n);
    
    }
    else
    {
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w20121",UNIT_MG_L,(float)(0.0),&arg_n);
		status = 1;
    }
