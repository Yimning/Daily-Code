...
Description:protocol_COD_GJSurfaceWater
...
int protocol_COD_GJSurfaceWater(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GJSurfaceWater COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GJSurfaceWater COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GJSurfaceWater COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GJSurfaceWater COD RECV:",com_rbuf,size);
...
Author:Yimning
Time:2021.08.02 Mon.
Description:protocol_COD_GJSurfaceWater_info
...
*/
int protocol_COD_GJSurfaceWater_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	cmd = 0x03;
	dataType = FLOAT_CDAB;
	SYSLOG_DBG("protocol COD GJSurfaceWater info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol COD GJSurfaceWater info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol COD GJSurfaceWater, Info1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol COD GJSurfaceWater Info1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol COD GJSurfaceWater info RECV1:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol COD GJSurfaceWater info SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol COD GJSurfaceWater, Info2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol COD GJSurfaceWater Info2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol COD GJSurfaceWater info RECV2:",com_rbuf,size);
