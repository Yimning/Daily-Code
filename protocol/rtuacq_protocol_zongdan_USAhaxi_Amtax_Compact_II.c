...
Author:Yimning
Time:2021.08.04 Wed.
Description:protocol_ZONGDAN_USAHaXi_maxII_New
...
int protocol_ZONGDAN_USAHaXi_maxII_New(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"USAHaXi maxII ZONGDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("USAHaXi maxII ZONGDAN protocol,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("USAHaXi maxII ZONGDAN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"USAHaXi maxII ZONGDAN RECV:",com_rbuf,size);
...
Author:Yimning
Time:2021.08.04 Wed.
Description:protocol_ZONGDAN_USAHaXi_maxII_New_info
...
DataType and Analysis:
	(FLOAT_CDAB) 85 20 3D 5D   = 0.054
*/
int protocol_ZONGDAN_USAHaXi_maxII_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_USAHaXi_maxII_New_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"USAHaXi maxII ZONGDAN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	dataType = FLOAT_CDAB;
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("USAHaXi maxII ZONGDAN INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("USAHaXi maxII ZONGDAN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"USAHaXi maxII ZONGDAN INFO RECV:",com_rbuf,size);
