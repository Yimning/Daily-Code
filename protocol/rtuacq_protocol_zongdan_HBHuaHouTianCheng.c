...
int protocol_ZONGDAN_HBHuaHouTianCheng(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,devaddr,0x03,0x7532,0x02);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHuaHouTianCheng TN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHuaHouTianCheng TN protocol,TN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHuaHouTianCheng TN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHuaHouTianCheng TN RECV:",com_rbuf,size);
...
int protocol_ZONGDAN_HBHuaHouTianCheng_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_HBHuaHouTianCheng_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHuaHouTianCheng TN INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHuaHouTianCheng TN INFO protocol,INFO1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHuaHouTianCheng TN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHuaHouTianCheng TN INFO RECV1:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHuaHouTianCheng TN INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHuaHouTianCheng TN INFO protocol,INFO2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHuaHouTianCheng TN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHuaHouTianCheng TN INFO RECV2:",com_rbuf,size);
