...
int protocol_VOCs_ChuangChi6(struct acquisition_data *acq_data)
{
#define POLCODE_NUM 8

	char *polcode[POLCODE_NUM]={
		"e30006","e30106","e70006","e70106",
		"g28006","g28206","g20106","g29006"
	};
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ChuangChi VOC SEND6:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ChuangChi VOC protocol,VOC 6: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ChuangChi VOC data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ChuangChi VOC RECV6:",com_rbuf,size);
