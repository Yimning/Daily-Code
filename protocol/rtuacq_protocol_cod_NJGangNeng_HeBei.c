...
int protocol_COD_NJGangNeng_HeBei(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, 0x0B, 0x03, 0x06D7, 0x02);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJGangNeng COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf ,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJGangNeng HeBei COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJGangNeng HeBei COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJGangNeng COD RECV:",com_rbuf,size);
...
int protocol_COD_NJGangNeng_HeBei_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_NJGangNeng_HeBei_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJGangNeng COD INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJGangNeng COD INFO protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJGangNeng COD INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJGangNeng COD INFO RECV:",com_rbuf,size);
