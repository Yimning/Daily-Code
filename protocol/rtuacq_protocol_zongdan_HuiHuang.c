...
Description:protocol_ZONGDAN_HuiHuang
TX:%01#RDD0152101525**
...
int protocol_ZONGDAN_HuiHuang(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuang zongdan SEND:",com_tbuf,size);
	size =write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuang zongdan RECV:",com_rbuf,size);
	SYSLOG_DBG("HuiHuang zongdan protocol,zongdan : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("HuiHuang zongdan data: %s",com_rbuf);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=29)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&(check!=NULL))
	{
		strncpy(data_buff,&com_rbuf[18],8);
		tem=atoi(data_buff);
		zongdan=((float)tem)/100;
	}
	else
	{
		status=1;
	...
	ret=acqdata_set_value(acq_data,"w21001",UNIT_MG_L,zongdan,&arg_n);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan Flag SEND:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ANDAN protocol,Flag : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan Flag RECV:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=20)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
		...
		switch(val){
			case 0: flag='N'; break;
			case 256: flag='D'; break;
			case 768: flag='C'; break;
			case 1024: flag='M'; break;
			case 2560: flag='K'; break;
...
Email:1148967988@qq.com
Create Time:2021.08.07 Sun.
Description:protocol_ZONGDAN_HuiHuang_info
...
*/
protocol_ZONGDAN_HuiHuang_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	char com_rbuf[2048]={0}; 
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND1:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV1:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=20)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
		SYSLOG_DBG("p= %s\n",p);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND2:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV2:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND3:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV3:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND4:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO4 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV4:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND5:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO5 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV5:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND6:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO6 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV6:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND7:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO7 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV7:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"JSHuiHuan INFO SEND8:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO8 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"JSHuiHuan INFO RECV8:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=12)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
