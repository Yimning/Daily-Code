...
Description:protocol_CEMS_SDXinZe_V2
...
int protocol_CEMS_SDXinZe_V2(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SDXinZe_V2 CEMS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SDXinZe_V2 CEMS protocol,CEMS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SDXinZe_V2 CEMS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SDXinZe_V2 CEMS RECV:",com_rbuf,size);
	...
	if(p!=NULL)
	{
		...
		O2 = getFloatValue(p, 15, dataType);
...
int protocol_CEMS_SDXinZe_V2_STATUS_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_SDXinZe_V2_STATUS_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SDXinZe_V2 STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SDXinZe_V2 CEMS protocol,STATUS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SDXinZe_V2 STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SDXinZe_V2 STATUS RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
	        val = getInt16Value(p, 31, INT_AB);
...
int protocol_CEMS_SDXinZe_V2_SO2_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_SDXinZe_V2_SO2_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SDXinZe_V2 SO2 INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SDXinZe_V2 SO2 protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SDXinZe_V2 SO2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SDXinZe_V2 SO2 INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		...
		modbus_ascii_pack_to_hex_pack(&p[63],16,pt); //32 30 31 39 30 39 30 37 31 32 33 34 35 36(ascii) //2019-09-07-12-34-56
		...
		//valf= getFloatValue(p, 129, dataType);
...
int protocol_CEMS_SDXinZe_V2_NO_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_SDXinZe_V2_NO_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SDXinZe_V2 NO INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SDXinZe_V2 NO protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SDXinZe_V2 NO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SDXinZe_V2 NO INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		...
		valf = getFloatValue(p, 85, dataType);
		acqdata_set_value_flag(acq_data,"i13023",UNIT_MG_M3,valf,INFOR_ARGUMENTS,&arg_n);
		
		memset(pt,0,sizeof(pt));
		modbus_ascii_pack_to_hex_pack(&p[89],16,pt); //32 30 31 39 30 39 30 37 31 32 33 34 35 36(ascii) //2019-09-07-12-34-56
		SYSLOG_DBG("%02x %02x %02d %02x %02d %02x %02x\n",pt[0],pt[1],BCD(pt[2]),pt[3],BCD(pt[4]),pt[5],pt[6]);
		val = getInt16Value(pt, 0, INT_AB);
		t1 = getTimeValue(BCD(val),  BCD(pt[2]), BCD(pt[3]), BCD(pt[4]),  BCD(pt[5]), BCD(pt[6]));
		acqdata_set_value_flag(acq_data,"i13021",t1,0,INFOR_ARGUMENTS,&arg_n);
...
int protocol_CEMS_SDXinZe_V2_O2_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_SDXinZe_V2_O2_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"SDXinZe_V2 O2 INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("SDXinZe_V2 O2 protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("SDXinZe_V2 O2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"SDXinZe_V2 O2 INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		...
		memset(pt,0,sizeof(pt));
		modbus_ascii_pack_to_hex_pack(&p[115],16,pt); //32 30 31 39 30 39 30 37 31 32 33 34 35 36(ascii) //2019-09-07-12-34-56
		SYSLOG_DBG("%02x %02x %02d %02x %02d %02x %02x\n",pt[0],pt[1],BCD(pt[2]),pt[3],BCD(pt[4]),pt[5],pt[6]);
		val = getInt16Value(pt, 0, INT_AB);
		t1 = getTimeValue(BCD(val),  BCD(pt[2]), BCD(pt[3]), BCD(pt[4]),  BCD(pt[5]), BCD(pt[6]));
		acqdata_set_value_flag(acq_data,"i13021",t1,0,INFOR_ARGUMENTS,&arg_n);
		...
		//valf= getFloatValue(p, 153, dataType);
		
		valf= getFloatValue(p, 157, dataType);
		acqdata_set_value_flag(acq_data,"i13024",UNIT_NONE,valf,INFOR_ARGUMENTS,&arg_n);
		
		valf = getFloatValue(p, 161, dataType);
		acqdata_set_value_flag(acq_data,"i13029",UNIT_NONE,valf,INFOR_ARGUMENTS,&arg_n);
