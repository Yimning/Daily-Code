...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TN protocol,TN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN RECV:",com_rbuf,size);
	...
	{
		size=read_device(DEV_DI3,(char *)(&di),4);
		SYSLOG_DBG("NJHongGuang TN DI protocol,TN DI: read device %s ,size=%d, di=%d\n",DEV_DI3,size,di);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN DI RECV:",&di,size);
		if(size>=4 && di==1)
		{
			dev_stat='P';
		}
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
		...
		LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TN SEND2:",com_tbuf,size);
		size=write_device(DEV_NAME(acq_data),com_tbuf,size);
		sleep(2);
		memset(com_rbuf,0,sizeof(com_rbuf));
		size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
		SYSLOG_DBG("NJHongGuang TN protocol,TN 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
		SYSLOG_DBG_HEX("NJHongGuang TN data",com_rbuf,size);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN RECV2:",com_rbuf,size);
...
int protocol_ZONGDAN_NJHongGuang_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TN INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TN protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN INFO RECV1:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, 0x3, 0x03);
	if(p!= NULL)
	{
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TN INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TN protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN INFO RECV2:",com_rbuf,size);
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang TN INFO SEND3:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(3);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang TN protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang TN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang TN INFO RECV3:",com_rbuf,size);
