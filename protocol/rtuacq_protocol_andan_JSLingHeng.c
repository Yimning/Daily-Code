...
int protocol_ANDAN_JSLingHeng(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,devaddr,cmd,0x3A98,cnt);
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "JSLingHeng ANDAN", 1);
	...
	if(p!=NULL)
	{
		...
		switch(val)
		{
			case 1: acq_data->dev_stat='M';break;
			case 2: acq_data->dev_stat='D';break;
			case 3: acq_data->dev_stat='M';break;
			case 4: acq_data->dev_stat='N';break;
			case 5: acq_data->dev_stat='K';break;
			default:acq_data->dev_stat='N';break;
		}

		andan=getFloatValue(p, 5, FLOAT_CDAB);
...
int protocol_ANDAN_JSLingHeng_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ANDAN_JSLingHeng_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSLingHeng ANDAN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSLingHeng ANDAN INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSLingHeng ANDAN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSLingHeng ANDAN INFO RECV:",com_rbuf,size);
