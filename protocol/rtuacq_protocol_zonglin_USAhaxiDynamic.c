...
int protocol_ZONGLIN_USAhaxiDynamic(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"haxi dynamic TP SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(2);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("haxi dynamic TP protocol,TP : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("haxi dynamic TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"haxi dynamic TP RECV:",com_rbuf,size);
