...
*/
	...
	SYSLOG_DBG("JSJinBiao_in speed  protocol,speed : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSJinBiao_in speed  data ",com_rbuf,size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSJinBiao_in total protocol,total : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSJinBiao_in total data ",com_rbuf,size);
	...
	acqdata_set_value(acq_data,"ea0401",UNIT_M3_H,speed,&arg_n);
