...
int protocol_level_MiKe_P260(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,devaddr,0x04,0x00,0x01);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"MiKe P260 level SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("MiKe P260 level protocol,level : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("MiKe P260 level data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"MiKe P260 level RECV:",com_rbuf,size);
