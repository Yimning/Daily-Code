...
int protocol_ANDAN_USAhaxiDynamic(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"haxi dynamic andan SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(2);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("haxi dynamic andan protocol,andan : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("haxi dynamic andan data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"haxi dynamic andan RECV:",com_rbuf,size);
	if((size>=37)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		...
		switch(val)
		{
			case 0: acq_data->instrstat=1;break;
			case 1: acq_data->instrstat=3;break;
			case 2: acq_data->instrstat=3;break;
			case 3: acq_data->instrstat=2;break;
			default: acq_data->instrstat=1;break;
		}
