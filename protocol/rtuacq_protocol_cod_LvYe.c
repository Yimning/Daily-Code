...
int protocol_COD_LvYe(struct acquisition_data *acq_data)
{
/*
...
*/
	...
	SYSLOG_DBG("LvYe COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LvYe COD data",com_rbuf,size);
	if(size>=11)
	{
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("LvYe COD status protocol,COD status: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LvYe COD status data",com_rbuf,size);
	if(size>=2)
	{
		val=com_rbuf[0];
		switch(val)
