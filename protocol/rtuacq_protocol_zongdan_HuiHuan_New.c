...
Description:protocol_ZONGDAN_HuiHuan
TX:%01#RDD0152101525**
RX:%01$RD1202020621400000054712
DataType and Analysis:
	(Float) 
*/
int protocol_ZONGDAN_HuiHuan_New(struct acquisition_data *acq_data)
{
	int status=0;
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN SEND:",com_tbuf,size);
	size =write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN RECV:",com_rbuf,size);
	SYSLOG_DBG("HuiHuan ZONGDAN protocol,ZONGDAN: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("HuiHuan ZONGDAN data: %s",com_rbuf);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=33)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&(check!=NULL))
	{
		strncpy(data_buff,&com_rbuf[22],8);
		tem=atoi(data_buff);
		zongdan=((float)tem)/100.0;
...
Email:1148967988@qq.com
Create Time:2021.08.07 Sun.
Description:protocol_ZONGDAN_HuiHuan_New_info
...
*/
protocol_ZONGDAN_HuiHuan_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	char com_rbuf[2048]={0}; 
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND1:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ZONGDAN protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV1:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND2:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuiHuan ZONGDAN protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV2:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND3:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV3:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND4:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO4 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV4:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND5:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO5 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV5:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND6:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO6 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV6:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND7:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO7 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV7:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	...
	size=strlen(com_tbuf);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuan ZONGDAN INFO SEND8:",com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGDAN protocol,INFO8 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuan ZONGDAN INFO RECV8:",com_rbuf,size);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=12)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&((p=memstr(com_rbuf, size, "%01$RD"))!=NULL)&&(check!=NULL))
	{
