...
DataType and Analysis:
	(FLOAT_ABCD) 
*/
static char TSP_AnRomhXin_LSS2004_LSS2014_Mark = 'N';
int protocol_TSP_AnRomhXin_LSS2004_LSS2014(struct acquisition_data *acq_data)
{
	...
	size=10;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"AnRomhXin LSS2004/LSS2014 TSP SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AnRomhXin LSS2004/LSS2014 TSP protocol,TSP : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("AnRomhXin LSS2004/LSS2014 TSP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"AnRomhXin LSS2004/LSS2014 TSP RECV:",com_rbuf,size);
	...
	if(status == 0){
		...
		acq_data->dev_stat = TSP_AnRomhXin_LSS2004_LSS2014_Mark;
	}
	else 
...
int protocol_TSP_AnRomhXin_LSS2004_LSS2014_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_TSP_AnRomhXin_LSS2004/LSS2014_info\n");
	...
	size=10;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"AnRomhXin LSS2004/LSS2014 TSP STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AnRomhXin LSS2004/LSS2014 TSP STATUS protocol,STATUS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("AnRomhXin LSS2004/LSS2014 TSP STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"AnRomhXin LSS2004/LSS2014 TSP STATUS RECV:",com_rbuf,size);
	if((size>=36)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x66)&&(com_rbuf[2]==0x1E))
	{
	...
	size=6;
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"AnRomhXin LSS2004/LSS2014 TSP INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AnRomhXin LSS2004/LSS2014 TSP protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("AnRomhXin LSS2004/LSS2014 TSP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"AnRomhXin LSS2004/LSS2014 TSP INFO RECV:",com_rbuf,size);
