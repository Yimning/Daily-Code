...
static float COD_Keruida_orig=0;
/*
Author:Yimning
Email:1148967988@qq.com
Create Time:2021.06.21 Fri.
Description:protocol_COD_HBKeruida
...
int protocol_COD_HBKeruida(struct acquisition_data *acq_data)
{
	...
	SYSLOG_DBG("COD_HBKeruida protocol : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("COD_HBKeruida protocol",com_rbuf,size);
	...
	acqdata_set_value(acq_data,"w01018",UNIT_MG_L,total_andan,&arg_n);	
...
Email:1148967988@qq.com
Create Time:2021.06.21 Fri.
Description:protocol_COD_HBKeruida_info
...
*/	
int protocol_COD_HBKeruida_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	devaddr=modbusarg->devaddr&0xffff;
	SYSLOG_DBG("protocol_COD_HBKeruida_info\n");
	acqdata_set_value(acq_data,"w01018",UNIT_MG_L,total_andan,&arg_n);	
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf, devaddr, cmd, regpos, regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_COD_HBKeruida_info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_COD_HBKeruida_info1,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_COD_HBKeruida_info1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_COD_HBKeruida_info RECV1:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!= NULL)
	{
		val = getInt16Value(p,3,INT_AB);
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf, devaddr, cmd, regpos, regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_COD_HBKeruida_info SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_COD_HBKeruida_info2,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_COD_HBKeruida_info2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_COD_HBKeruida_info RECV2:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!= NULL)
	{
		val = getInt16Value(p,3,INT_AB);
		...
		valf = getFloatValue(p, 27, dataType);//COD_Keruida_orig
		COD_Keruida_orig=valf;   
    		acqdata_set_value_flag(acq_data,"i13105",UNIT_NONE,COD_Keruida_orig,INFOR_ARGUMENTS,&arg_n);
