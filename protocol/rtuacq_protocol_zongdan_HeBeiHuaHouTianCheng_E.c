...
Description:protocol_ZONGDAN_HeBeiHuaHouTianCheng_E
...
int protocol_ZONGDAN_HeBeiHuaHouTianCheng_E(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HuaHouTianCheng HeBei ZONGDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuaHouTianCheng HeBei ZONGDAN protocol,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HuaHouTianCheng HeBei ZONGDAN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HuaHouTianCheng HeBei ZONGDAN RECV:",com_rbuf,size);
...
int protocol_ZONGDAN_HeBeiHuaHouTianCheng_E_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_HeBeiHuaHouTianCheng_E_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd ,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HuaHouTianCheng HeBei ZONGDAN STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuaHouTianCheng HeBei ZONGDAN protocol,STATUS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HuaHouTianCheng HeBei STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HuaHouTianCheng HeBei ZONGDAN STATUS RECV:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p !=NULL)
	{
		val = getInt16Value(p, 3, INT_AB);
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd ,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HuaHouTianCheng HeBei ZONGDAN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HuaHouTianCheng HeBei ZONGDAN protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HuaHouTianCheng HeBei INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HuaHouTianCheng HeBei ZONGDAN INFO RECV:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf, size, devaddr, cmd, regcnt);
	if(p !=NULL)
	{
