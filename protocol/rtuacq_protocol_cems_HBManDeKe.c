...
int protocol_CEMS_HBManDeKe_SO2_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	status=1;
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,0x3,0x0C,0x6F);
	...
	if((size>=227)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		val=com_rbuf[185];
		val<<=8;
		val+=com_rbuf[186];
		...
		timer.tm_mon=com_rbuf[204]-1;
		timer.tm_mday=com_rbuf[206];
		timer.tm_hour=com_rbuf[208];
		timer.tm_min=com_rbuf[210];
		timer.tm_sec=com_rbuf[212];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
		...
		val+=com_rbuf[214];
		timer.tm_year=val-1900;
		timer.tm_mon=com_rbuf[216]-1;
		timer.tm_mday=com_rbuf[218];
		timer.tm_hour=com_rbuf[220];
		timer.tm_min=com_rbuf[222];
		timer.tm_sec=com_rbuf[224];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
...
int protocol_CEMS_HBManDeKe_NOx_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBManDeKe NOx INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBManDeKe NOx protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBManDeKe NOx data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBManDeKe NOx INFO RECV:",com_rbuf,size);
	if((size>=243)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		f.ch[3]=com_rbuf[5];
		f.ch[2]=com_rbuf[6];
		f.ch[1]=com_rbuf[3];
		...
		val+=com_rbuf[206];
		timer.tm_year=val-1900;
		timer.tm_mon=com_rbuf[208]-1;
		timer.tm_mday=com_rbuf[210];
		timer.tm_hour=com_rbuf[212];
		timer.tm_min=com_rbuf[214];
		timer.tm_sec=com_rbuf[216];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
		acqdata_set_value_flag(acq_data,"i13021",t3,0.0,INFOR_ARGUMENTS,&arg_n);
		...
		val+=com_rbuf[230];
		timer.tm_year=val-1900;
		timer.tm_mon=com_rbuf[232]-1;
		timer.tm_mday=com_rbuf[234];
		timer.tm_hour=com_rbuf[236];
		timer.tm_min=com_rbuf[238];
		timer.tm_sec=com_rbuf[240];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
		acqdata_set_value_flag(acq_data,"i13027",t3,0.0,INFOR_ARGUMENTS,&arg_n);
...
int protocol_CEMS_HBManDeKe_O2_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if((size>=247)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		f.ch[3]=com_rbuf[5];
		f.ch[2]=com_rbuf[6];
		f.ch[1]=com_rbuf[3];
		...
		val+=com_rbuf[198];
		timer.tm_year=val-1900;
		timer.tm_mon=com_rbuf[200]-1;
		timer.tm_mday=com_rbuf[202];
		timer.tm_hour=com_rbuf[204];
		timer.tm_min=com_rbuf[206];
		timer.tm_sec=com_rbuf[208];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
		acqdata_set_value_flag(acq_data,"i13021",t3,0.0,INFOR_ARGUMENTS,&arg_n);
		...
		val+=com_rbuf[234];
		timer.tm_year=val-1900;
		timer.tm_mon=com_rbuf[236]-1;
		timer.tm_mday=com_rbuf[238];
		timer.tm_hour=com_rbuf[240];
		timer.tm_min=com_rbuf[242];
		timer.tm_sec=com_rbuf[244];
		t2=mktime(&timer);
		t3=(t2>t1)? (t2-t1):t1;
		acqdata_set_value_flag(acq_data,"i13027",t3,0.0,INFOR_ARGUMENTS,&arg_n);
