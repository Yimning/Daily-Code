...
{
/*
	TX:	09 03 00 00 00 02 ** **
	RX:	09 03 04 57 0A 42 95 FF FF
...
*/
	...
	size=modbus_pack(com_tbuf, 0x09, 0x03, 0x00, 0x02);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJRuiQuan modbus zonglin protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJRuiQuan modbus zonglin data ",com_rbuf,size);	
...
int protocol_ZONGLIN_NJRuiQuanModbus_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
