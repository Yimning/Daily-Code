...
int protocol_VOCs_ChuangTuo2(struct acquisition_data *acq_data)
{
#define POLCODE_NUM 18

	char *polcode[POLCODE_NUM]={
		"g29001","g28001","","","g18401",
		"e80201","e70201","e70202","","e60201",
		"","","","","",
		"g20101","g09001","g19501"
	};
/*
...
*/
	UNIT_DECLARATION unit[POLCODE_NUM]={
		UNIT_MG_M3,		UNIT_0C,		UNIT_NONE,	UNIT_NONE,	UNIT_KPA,		
		UNIT_A,   		UNIT_A,		UNIT_A,		UNIT_NONE,	UNIT_A,
		UNIT_NONE,		UNIT_NONE,	UNIT_NONE,	UNIT_NONE,	UNIT_NONE,	
		UNIT_M3_S,		UNIT_MG_M3,	UNIT_PECENT
	};
	...
	int arg_n=0;
	char *p;
	char head[4]={0x01,0x03,0x48,0x00};
	int devaddr=0;
	...
	size=modbus_pack(com_tbuf,devaddr,0x03,0x00,0x24);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ChuangTuo2 voc SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ChuangTuo2 voc protocol,voc : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ChuangTuo2 voc data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ChuangTuo2 voc RECV:",com_rbuf,size);
