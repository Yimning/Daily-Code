...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	{
		size=read_device(DEV_DI1,(char *)(&di),4);
		SYSLOG_DBG("NJHongGuang NH3N DI protocol,NH3N DI: read device %s ,size=%d, di=%d\n",DEV_DI1,size,di);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang NH3N DI RECV:",&di,size);
		if(size>=4 && di==1)
		{
			dev_stat='P';
		}
...
int protocol_ANDAN_NJHongGuang_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	size=modbus_pack(com_tbuf,devaddr,0x3,regpos,regcnt);
