...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(2);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD RECV:",com_rbuf,size);
	...
	{
		size=read_device(DEV_DI0,(char *)(&di),4);
		SYSLOG_DBG("NJHongGuang COD DI protocol,COD DI: read device %s ,size=%d, di=%d\n",DEV_DI0,size,di);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD DI RECV:",&di,size);
		if(size>=4 && di==1)
		{
			dev_stat='P';
		}
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
		...
		LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang COD SEND2:",com_tbuf,size);
		size=write_device(DEV_NAME(acq_data),com_tbuf,size);
		sleep(2);
		memset(com_rbuf,0,sizeof(com_rbuf));
		size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
		SYSLOG_DBG("NJHongGuang COD protocol,COD 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
		SYSLOG_DBG_HEX("NJHongGuang COD data",com_rbuf,size);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD RECV2:",com_rbuf,size);
...
int protocol_COD_NJHongGuang_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_NJHongGuang_info\n");
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang COD INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang COD protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD INFO RECV1:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, 0x3, 0x03);
	if(p!= NULL)
	{
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang COD INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang COD protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD INFO RECV2:",com_rbuf,size);
...
//ret=config_device(DEV_NAME(acq_data),SERIAL_TCIOFLUSH_DATA,NULL); //clear buffer
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJHongGuang COD INFO SEND3:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(3);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJHongGuang COD protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJHongGuang COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJHongGuang COD INFO RECV3:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, 0x3, 0xA4);
	//if(p!= NULL)
	//{
	if((size>=333)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		val=com_rbuf[3];
		val<<=8;
		val+=com_rbuf[4];
