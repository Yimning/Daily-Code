...
int protocol_ZONGLIN_NJGangNeng_HeBei(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf, 0x0D, 0x03, 0x06D7, 0x02);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJGangNeng TP SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf ,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJGangNeng HeBei TP protocol,TP : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJGangNeng HeBei TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJGangNeng TP RECV:",com_rbuf,size);
	if(size>=9 && com_rbuf[0]==0x0D && com_rbuf[1]==0x03)
	{
		f.ch[3]=com_rbuf[5];
		f.ch[2]=com_rbuf[6];
		f.ch[1]=com_rbuf[3];
...
int protocol_ZONGLIN_NJGangNeng_HeBei_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGLIN_NJGangNeng_HeBei_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJGangNeng TP INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJGangNeng TP INFO protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJGangNeng TP INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJGangNeng TP INFO RECV:",com_rbuf,size);
