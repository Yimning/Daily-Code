...
Description:protocol_CEMS_HBHeFeng
TX:01 03 00 00 00 0C C5 C8
RX:01 03 18
41 20 00 00
42 48 00 00
41 A0 00 00
...
int protocol_CEMS_HBHeFeng(struct acquisition_data *acq_data)
{
	...
	size=modbus_pack(com_tbuf,devaddr,cmd,0x00,0x0C);
	...
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, 0x0C);
	if(p!=NULL)
	{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHeFeng CEMS Flag SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHeFeng CEMS Flag protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHeFeng CEMS Flag data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHeFeng CEMS Flag RECV:",com_rbuf,size);
...
RX2:01 03 00 C8 00 1C C5 FD
TX2:01 03 38 42 C8 00 00 20 20 10 16 08 00 00 00 00 00 00 00 00 00 00 00 3F 80 00 00 00 00 00 00 
20 20 10 16 09 00 00 00 42 C8 00 00 43 48 00 00 3F 80 00 00 00 00 00 00 3F 80 00 00 31 72
...
int protocol_CEMS_HBHeFeng_SO2_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_HBHeFeng_SO2_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,0x64,0x03);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHeFeng SO2 INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHeFeng SO2 protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHeFeng SO2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHeFeng SO2 INFO RECV1:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, 0x03);
	if(p!=NULL)
	{
	        val= getInt16Value(p, 3, INT_AB);
		...
		switch(val){
				...
				break;	
        		case 7:
				acqdata_set_value_flag(acq_data,"i12009",UNIT_NONE,7,INFOR_STATUS,&arg_n);
				break;	
        		case 8:
				acqdata_set_value_flag(acq_data,"i12009",UNIT_NONE,8,INFOR_STATUS,&arg_n);
				break;	
        		case 9:
				acqdata_set_value_flag(acq_data,"i12009",UNIT_NONE,9,INFOR_STATUS,&arg_n);
				break;	
        		default:
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHeFeng SO2 INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHeFeng SO2 protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHeFeng SO2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHeFeng SO2 INFO RECV2:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, 0x1C);
	if(p!=NULL)
	{
...
RX:01 03 00 E4 00 1C 04 34
TX:01 03 38 42 C8 00 00 20 20 10 16 08 00 00 00 00 00 00 00 00 00 00 00 3F 80 00 00 00 00 00 00 
20 20 10 16 09 00 00 00 42 C8 00 00 43 48 00 00 3F 80 00 00 00 00 00 00 3F 80 00 00 31 72
DataType and Analysis:
	(INT_AB) 01 = 1
...
int protocol_CEMS_HBHeFeng_NO_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_HBHeFeng_NOx_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHeFeng NOx INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHeFeng NOx protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHeFeng NOx data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHeFeng NOx INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, 0x1C);
	if(p!=NULL)
	{
		valf = getFloatValue(p, 3, dataType);
...
DataType and Analysis:
	...
	(FLOAT_ABCD) 42 c8 00 00  = 100
*/
int protocol_CEMS_HBHeFeng_O2_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_CEMS_HBHeFeng_O2_info\n");
	...
	cmd = 0x03;
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,0x0100,0x1C);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"HBHeFeng O2 INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("HBHeFeng O2 protocol,INFO : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HBHeFeng O2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"HBHeFeng O2 INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, 0x1C);
	if(p!=NULL)
	{
		valf = getFloatValue(p, 3, dataType);
