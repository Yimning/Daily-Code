...
static float ZONGLIN_QDJiaMing_orig=0;
//rtuacq_protocol_cod_QDJiaMing.c
/*
Author:Yimning
Time:2021.07.20 Tues.
Description:protocol_ZONGLIN_QDJiaMing
...
int protocol_ZONGLIN_QDJiaMing(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"QDJiaMing ZONGLIN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("QDJiaMing ZONGLIN protocol,ZONGLIN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("QDJiaMing ZONGLIN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"QDJiaMing ZONGLIN RECV:",com_rbuf,size);
	...
	ret=acqdata_set_value(acq_data,"w21011",UNIT_MG_L,total_zonglin,&arg_n);
	if(status == 0)
...
Author:Yimning
Time:2021.07.20 Tues.
Description:protocol_ZONGLIN_QDJiaMing_info
...
*/
int protocol_ZONGLIN_QDJiaMing_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	cmd = 0x03;
	//dataType = INT_AB;
	SYSLOG_DBG("protocol_ZONGLIN_QDJiaMing_info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_ZONGLIN_QDJiaMing_info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_ZONGLIN_QDJiaMing_info,Info1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_ZONGLIN_QDJiaMing Info1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_ZONGLIN_QDJiaMing_info RECV1:",com_rbuf,size);
	if((size>=11)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==cmd))
	{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_ZONGLIN_QDJiaMing_info SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_ZONGLIN_QDJiaMing_info,Info2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_ZONGLIN_QDJiaMing Info2 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_ZONGLIN_QDJiaMing_info RECV2:",com_rbuf,size);
