...
int protocol_PLC_SDXinZe_v1_7(struct acquisition_data *acq_data)
{
	...
	int val=0;
	float valf = 0;
	float speed=0,temp=0,press=0,diff=0;
	...
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "SDXinZe PLC V1.7", 1);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, cnt);
	if(p!=NULL)
	{
		temp = getFloatValue(p, 3, dataType);
		press = getFloatValue(p, 7, dataType);
		diff = getFloatValue(p, 11, dataType);
		speed= getFloatValue(p, 15, dataType);
	...
	acqdata_set_value(acq_data,"a01017",UNIT_PA,diff,&arg_n);
