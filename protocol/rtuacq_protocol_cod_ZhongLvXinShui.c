...
int protocol_COD_ZhongLv(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD RECV1:",com_rbuf,200);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD RECV2:",&com_rbuf[200],size-200);
	...
	ret=acqdata_set_value_orig(acq_data,"w01018",UNIT_MG_L,total_cod,orig,&arg_n);
	if(status == 0)
...
int protocol_COD_ZhongLv_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZhongLv_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv COD STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD STATUS RECV:",com_rbuf,size);
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,0x3,0x02A4,0x6E);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"zhonglv COD INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	if(size>200)
	{
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD INFO RECV1:",com_rbuf,200);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD INFO RECV2:",&com_rbuf[200],size-200);
	}
	else
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"zhonglv COD INFO RECV:",com_rbuf,size);
	if((size>=225)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
