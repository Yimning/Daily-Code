...
*/
   ...
   float valf=0;
	...
	SYSLOG_DBG("HuaShiJie liujiage protocol,liujiage: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("HuaShiJie liujiage data\n",com_rbuf,size);
	...
	if((size>=22)&&(com_rbuf[0]==0x3e)&&(com_rbuf[1]==0x30)&&(com_rbuf[2]==0x31))
	{
		...
		if(com_rbuf[20]==0x30)valf*=1000000;
		if(com_rbuf[20]==0x31)valf*=1000;
		SYSLOG_DBG("HuaShiJie liujiage =%f\n",valf);
	}
	else
	{
	   valf=0;
	...
	ret=acqdata_set_value(acq_data,"w20117",UNIT_MG_L,valf,&arg_n);
	if(status == 0)
...
int protocol_LIUJIAGE_HNHuaShiJie(struct acquisition_data *acq_data)
{
   ...
   union uf f;
	...
	ret=acqdata_set_value(acq_data,"w20117",UNIT_MG_L,total_lin,&arg_n);
