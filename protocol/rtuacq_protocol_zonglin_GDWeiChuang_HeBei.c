...
static struct hebei_dynamic_info_water GDWeiChuang_TP_info;
...
#define INFOPOL_NUM 22
	...
	char polcodestr[POLCODE_NUM][20]={"w21011"};
	...
	SYSLOG_DBG("protocol_ZONGLIN_GDWeiChuang_HeBei:size=%d,%s\n",POLCODE_NUM,polcodestr[0]);
	info_data=&GDWeiChuang_TP_info;
...
int protocol_ZONGLIN_GDWeiChuang_HeBei_info(struct acquisition_data *acq_data)
{
	int arg_n=0;
	struct hebei_dynamic_info_water *info_data;
	static int num=0;

	SYSLOG_DBG("protocol_ZONGLIN_GDWeiChuang_HeBei_info\n");
