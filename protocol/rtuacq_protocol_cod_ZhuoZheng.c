...
int protocol_COD_ShiHuiLin(struct acquisition_data *acq_data)
{
   ...
   LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ShiHuiLin COD SEND:",com_tbuf,size);
   size=write_device(DEV_NAME(acq_data),com_tbuf,size);
   sleep(2);

   	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ShiHuiLin COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	print_hex(com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ShiHuiLin COD RECV:",com_rbuf,size);
...
*/
		...
		f.ch[3] = com_rbuf[3];
		f.ch[2] = com_rbuf[4];
	SYSLOG_DBG("cod data:%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,\n",
	com_rbuf[0],com_rbuf[1],com_rbuf[2],com_rbuf[3],
	com_rbuf[4],com_rbuf[5],com_rbuf[6],com_rbuf[7],com_rbuf[8]);
		
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w01018",UNIT_MG_L,(float)(f.f),&arg_n);
    
    }
    else
    {
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w01018",UNIT_MG_L,(float)(0.0),&arg_n);
		status = 1;
    }
...
int protocol_COD_ZhuoZheng2(struct acquisition_data *acq_data)
{
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("zhuozheng COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("zhuozheng COD",com_rbuf,size);
