...
int protocol_ZONGLIN_JSLingHeng(struct acquisition_data *acq_data)
{
	...
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "JSLingHeng TP", 1);
	...
	if(p!=NULL)
	{
		...
		tp=getFloatValue(p, 5, FLOAT_CDAB);
...
int protocol_ZONGLIN_JSLingHeng_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGLIN_JSLingHeng_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSLingHeng TP INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSLingHeng TP INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSLingHeng TP INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSLingHeng TP INFO RECV:",com_rbuf,size);
