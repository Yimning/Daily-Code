...
Description:protocol_ZONGLIN_HuiHuang
TX:%01#RDD0152101525**
...
int protocol_ZONGLIN_HuiHuang(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"HuiHuang zonglin SEND:",com_tbuf,size);
	size =write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"HuiHuang zonglin RECV:",com_rbuf,size);
	SYSLOG_DBG("HuiHuang zonglin protocol,zonglin : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("HuiHuang zonglin data: %s",com_rbuf);
	check = BCC_Check_OK(string_ok, com_rbuf,size);
	if((size>=29)&&(com_rbuf[0]=='%')&&(com_rbuf[1]=='0')&&(check!=NULL))
	{
		strncpy(data_buff,&com_rbuf[18],8);
		tem=atoi(data_buff);
		zonglin=((float)tem)/1000;
	}
	else
	{
		status=1;
...
Email:1148967988@qq.com
Create Time:2021.08.07 Sun.
Description:protocol_ZONGLIN_HuiHuang_info
...
*/
protocol_ZONGLIN_HuiHuang_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	char com_rbuf[2048]={0}; 
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO2 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO3 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO4 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO5 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO6 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO7 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSHuiHuan ZONGLIN protocol,INFO8 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
