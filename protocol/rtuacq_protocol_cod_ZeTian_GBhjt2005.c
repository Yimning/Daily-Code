...
int protocol_COD_ZeTian_GBhjt2005(struct acquisition_data *acq_data)
{
	...
	char flag=0;
	int ret=0;
	static char polcode[10]="011";
	char str[50];

	if(!acq_data) return -1; 

	size=getStrDataPack(DEV_NAME(acq_data), NULL, 0, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD", 1);
	if((p=strstr(com_rbuf,"CN=2011"))!=NULL)
	{
		q=p;
		if(((p=strstr(p,"CP=&&"))==NULL) || (strstr(&p[5],"&&")==NULL))
		{
			size+=getStrDataPack(DEV_NAME(acq_data), NULL, 0, &com_rbuf[size], sizeof(com_rbuf)-size, "ZeTian GBhjt2005 COD again", 1);
			p=strstr(q,"CP=&&");
		}
...
int protocol_COD_ZeTian_GBhjt2005_status(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_status\n");
	
	acq_ctrl=ACQ_CTRL(acq_data);
	modbusarg=&acq_ctrl->modbusarg_running;

	acqdata_set_value_flag(acq_data,"w01018",UNIT_NONE,0,INFOR_ARGUMENTS,&arg_n);
	
	memset(com_tbuf,0,sizeof(com_tbuf));
	strcpy(com_tbuf,"##0062ST=32;CN=5015;PW=123456;MN=00000000000001;AL=;CP=&&DataTime=&&C181\r\n");
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD status", 1);
...
int protocol_COD_ZeTian_GBhjt2005_info1(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_info1\n");
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info1.1", 1);
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info1.2", 1);
...
int protocol_COD_ZeTian_GBhjt2005_info2(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_info2\n");
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info2.1", 1);
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info2.2", 1);
...
int protocol_COD_ZeTian_GBhjt2005_info3(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_info3\n");
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info3.1", 1);
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info3.2", 1);
...
int protocol_COD_ZeTian_GBhjt2005_info4(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_info4\n");
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info4.1", 1);
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info4.2", 1);
...
int protocol_COD_ZeTian_GBhjt2005_info5(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_ZeTian_GBhjt2005_info5\n");
	...
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info5.1", 1);
	...
	sleep(1);
	memset(com_tbuf,0,sizeof(com_tbuf));
	strcpy(com_tbuf,"##0067ST=32;CN=5025;PW=123456;MN=00000000000001;K2Cr2o7=;CP=&&DataTime=&&1100\r\n");
	size=strlen(com_tbuf);
	size=getStrDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "ZeTian GBhjt2005 COD info5.2", 1);
	if(((p=strstr(com_rbuf,"CN=5025"))!=NULL) && ((p=strstr(p,";K2Cr2o7="))!=NULL))
	{
		sscanf(p,";K2Cr2o7=%f;",&valf);
		acqdata_set_value_flag(acq_data,"i13127",UNIT_MG_L,valf,INFOR_ARGUMENTS,&arg_n);
	}
