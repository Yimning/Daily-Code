...
static float COD_QDJiaMing_orig=0;
//rtuacq_protocol_cod_QDJiaMing.c
/*
Author:Yimning
Time:2021.07.20 Tues.
Description:protocol_COD_QDJiaMing
...
int protocol_COD_QDJiaMing(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"QDJiaMing COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("QDJiaMing COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("QDJiaMing COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"QDJiaMing COD RECV:",com_rbuf,size);
...
Author:Yimning
Time:2021.07.20 Tues.
Description:protocol_COD_QDJiaMing_info
...
*/
int protocol_COD_QDJiaMing_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	cmd = 0x03;
	//dataType = INT_AB;
	SYSLOG_DBG("protocol_COD_QDJiaMing_info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_COD_QDJiaMing_info SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_COD_QDJiaMing_info,Info1 : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_COD_QDJiaMing Info1 data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_COD_QDJiaMing_info RECV1:",com_rbuf,size);
	if((size>=11)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==cmd))
	{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_COD_QDJiaMing_info SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_COD_QDJiaMing_info RECV2:",com_rbuf,size);
