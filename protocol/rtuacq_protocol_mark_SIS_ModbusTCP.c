...
RX1:00 01 00 00 00 03 01 01 08
...
#define MODBUS_POLCODE_NUM 5
	...
	length = length + 6;
	if((com_rbuf[0]==0x00)&&(com_rbuf[1]==0x01)&&(com_rbuf[2]==0x00)&&(com_rbuf[3]==0x00)&&(size == length)&&(com_rbuf[6]==devaddr)&&(com_rbuf[7]==cmd))
	{
...
for(i=0;i<MODBUS_POLCODE_NUM;i++)
{
	...
	#if 0
	   for(i =0 ; i < valIntLen ;i++)
	    {
		valInt[i] = com_rbuf[9+i];
		...
		if(valInt[i*6+0]==1)
			mark=MARK_DC_Fb;
		else if(valInt[i*6+1]==1)
			mark=MARK_DC_Sta;
		else if(valInt[i*6+2]==1)
			mark=MARK_DC_Stb;
		else if(valInt[i*6+3]==1)
			mark=MARK_DC_Sd;
		else if(valInt[i*6+4]==1)
			mark=MARK_DC_Fa;
		else if(valInt[i*6+5]==1)
			mark=MARK_DC_Sr;
		else 
			mark=MARK_N;
		acqdata_set_value(acq_data,polcodestrflag[i],UNIT_NONE,(float)mark,&arg_n);
	    }
	#endif
