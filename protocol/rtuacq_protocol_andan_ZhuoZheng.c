...
int protocol_ANDAN_ZhuoZheng(struct acquisition_data *acq_data)
{
   ...
   size=modbus_pack(com_tbuf,0x1,0x3,0x00,0x2);
    LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ShiHuiLin ANDAN SEND:",com_tbuf,size);
   size=write_device(DEV_NAME(acq_data),com_tbuf,size);
   sleep(1);

   	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("ZhuoZheng ANDAN protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZhuoZheng ANDAN data ",com_rbuf,size);
    LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ShiHuiLin ANDAN RECV:",com_rbuf,size);
	pf=acq_data->com_f;       
...
*/
		...
		f.ch[3] = com_rbuf[3];
		f.ch[2] = com_rbuf[4];
	SYSLOG_DBG("ANDAN data:%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,%.2x,\n",
	com_rbuf[0],com_rbuf[1],com_rbuf[2],com_rbuf[3],
	com_rbuf[4],com_rbuf[5],com_rbuf[6],com_rbuf[7],com_rbuf[8]);
		
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w21003",UNIT_MG_L,(float)(f.f),&arg_n);
    
    }
    else
    {
		read_system_time(acq_data->acq_tm);
		acqdata_set_value(acq_data,"w21003",UNIT_MG_L,(float)(0.0),&arg_n);
		status = 1;
    }
...
int protocol_ANDAN_ZhuoZheng2(struct acquisition_data *acq_data)
{
	...
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ZhuoZheng ANDAN2 protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZhuoZheng ANDAN2 data ",com_rbuf,size);
