...
int protocol_others_tengentTx(struct acquisition_data *acq_data)
{
   ...
   mpolcodearg=modbusarg_running->polcode_arg;
	...
	SYSLOG_DBG("tengentTx1 protocol : send1 device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("tengentTx1 send1 data",com_tbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"tengentTx SEND1:",com_tbuf,size);
	sleep(1);

    memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("tengentRx1 protocol : read1 device1 %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("tengentRx1 read1 data1",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"tengentRx RECV1:",com_rbuf,size);
#else
	com_tbuf[0] = 0x01;
	com_tbuf[1] = 0x03;
	com_tbuf[2] = 0x04;
	...
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	SYSLOG_DBG("tengentTx protocol : send device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("tengentTx send data",com_tbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"tengentTx SEND:",com_tbuf,size);
	sleep(1);
	

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("tengentTx protocol : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("tengentTx read data",com_rbuf,size);
