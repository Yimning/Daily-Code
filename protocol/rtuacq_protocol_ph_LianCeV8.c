...
Author:Yimning
Time:2021.08.30 Mon.
Description:protocol_ph_LianCeV8
TX: 01 03 00 01 00 04 15 C9
RX: 01 03 08 
41 03 32 33 
41 03 32 33 
D2 74
...
int protocol_ph_LianCeV8(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"LianCe8.0 PH SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf ,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data), com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("LianCe8.0 PH protocol,PH : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LianCe8.0 PH data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"LianCe8.0 PH RECV:",com_rbuf,size);
