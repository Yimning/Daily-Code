...
int protocol_VOCs_DGYongBang(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"DGYongBang voc SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("DGYongBang voc protocol,voc 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("DGYongBang voc data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"DGYongBang voc RECV1:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"DGYongBang voc SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("DGYongBang voc protocol,voc 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("DGYongBang voc data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"DGYongBang voc RECV2:",com_rbuf,size);
	if((size>=47)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==03))
	{
		...
		val<<=8;
		val+=com_rbuf[40];
		acqdata_set_value(acq_data,"e80101",UNIT_KW,val/100.0,&arg_n);
		...
		val<<=8;
		val+=com_rbuf[42];
		acqdata_set_value(acq_data,"e80001",UNIT_KWH,val/100.0,&arg_n);
	}
	else
	{
		...
		acqdata_set_value(acq_data,"e80101",UNIT_KW,0,&arg_n);
		acqdata_set_value(acq_data,"e80001",UNIT_KWH,0,&arg_n);
		status=1;
	}
