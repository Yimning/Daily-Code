...
*/
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"yuanjin ANDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("yuanjin modbus andan protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("yuanjin modbus andan data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"yuanjin ANDAN RECV:",com_rbuf,size);
