...
int protocol_ZONGDAN_AHWanYi(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"AHWanYi HeBei TN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHWanYi HeBei TN protocol,TN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("AHWanYi HeBei TN data : %s",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"AHWanYi HeBei TN RECV:",com_rbuf,size);
...
int protocol_ZONGDAN_AHWanYi_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_AHWanYi_info\n");
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"AHWanYi TN STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHWanYi TN STATUS protocol,STATUS : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("AHWanYi TN STATUS data : %s",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"AHWanYi TN STATUS RECV:",com_rbuf,size);
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"AHWanYi TN INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHWanYi TN INFO protocol,INFO 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("AHWanYi TN INFO data : %s",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"AHWanYi TN INFO RECV1:",com_rbuf,size);
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"AHWanYi TN INFO SEND2:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHWanYi TN INFO protocol,INFO 2: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("AHWanYi TN INFO data : %s",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"AHWanYi TN INFO RECV2:",com_rbuf,size);
	if(size>=19 && modbus_ascii_pack_check(com_rbuf, devaddr, cmd))
	{
		memset(p,0,sizeof(p));
		modbus_ascii_pack_to_hex_pack(com_rbuf,size,p);
	...
	LOG_WRITE_STR(DEV_NAME(acq_data),0,"AHWanYi TN INFO SEND3:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("AHWanYi TN INFO protocol,INFO 3: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("AHWanYi TN INFO data : %s",com_rbuf);
	LOG_WRITE_STR(DEV_NAME(acq_data),1,"AHWanYi TN INFO RECV3:",com_rbuf,size);
