...
static float ZONGDAN_LiDa_orig=0;

/*
Author:Yimning
Time:2021.06.09 Wed.
Description:protocol_ZONGDAN_LiDa
...
int protocol_ZONGDAN_LiDa(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"LiDa ZONGDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("LiDa ZONGDAN protocol,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LiDa ZONGDAN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"LiDa ZONGDAN RECV:",com_rbuf,size);
	...
	ret=acqdata_set_value_orig(acq_data,"w21001",UNIT_MG_L,total_ZONGDAN,ZONGDAN_LiDa_orig,&arg_n);
	if(status == 0)
...
Author:Yimning
Time:2021.06.09 Wed.
Description:protocol_ZONGDAN_LiDa_info
...
*/	
int protocol_ZONGDAN_LiDa_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	float ZONGDAN_zero=0,ZONGDAN_range=0,ZONGDAN_K=0,ZONGDAN_PPB=0;
	float ZONGDAN_correction_factor=0,ZONGDAN_adjust_zero_drift=0,ZONGDAN_range_drift=0,ZONGDAN_intercept=0,ZONGLIN_BDXS=0,cod_original_Val=0;
	float ZONGDAN_digestion_time=0,ZONGDAN_digestion_temp=0;
	...
	devaddr=modbusarg->devaddr&0xffff;
	SYSLOG_DBG("protocol_ZONGDAN_LiDa_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_ZONGDAN_LiDa_info SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_ZONGDAN_LiDa_info,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_ZONGDAN_LiDa_info data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_ZONGDAN_LiDa_info RECV:",com_rbuf,size);
	if((size>=75)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		...
		f.ch[2]=com_rbuf[32];
		f.ch[3]=com_rbuf[31];
		ZONGDAN_zero=f.f;   //ZONGDAN_zero_val
    	//acqdata_set_value_flag(acq_data,"i13104",UNIT_L,ZONGDAN_zero,INFOR_ARGUMENTS,&arg_n);

    	//acqdata_set_value_flag(acq_data,"i13105",UNIT_NONE,ZONGDAN_LiDa_orig,INFOR_ARGUMENTS,&arg_n);
	...
	//acqdata_set_value_flag(acq_data,"i13003",UNIT_MINUTE,val,INFOR_ARGUMENTS,&arg_n);
		...
		f.ch[2]=com_rbuf[54];
		f.ch[3]=com_rbuf[53];
		ZONGDAN_correction_factor=f.f;   //ZONGDAN_correction_factor
    	acqdata_set_value_flag(acq_data,"i13119",UNIT_NONE,ZONGDAN_correction_factor,INFOR_ARGUMENTS,&arg_n);
		...
		f.ch[2]=com_rbuf[58];
		f.ch[3]=com_rbuf[57];
		ZONGDAN_adjust_zero_drift=f.f;   //ZONGDAN_adjust_zero_drift
    	acqdata_set_value_flag(acq_data,"i13120",UNIT_NONE,ZONGDAN_adjust_zero_drift,INFOR_ARGUMENTS,&arg_n);	

	sprintf(hexarry,"%x%x",com_rbuf[59],com_rbuf[60]);
	ZONGDAN_digestion_time = atoi(hexarry);
	acqdata_set_value_flag(acq_data,"i13005",UNIT_MINUTE,ZONGDAN_digestion_time,INFOR_ARGUMENTS,&arg_n);	
		...
		f.ch[2]=com_rbuf[64];
		f.ch[3]=com_rbuf[63];
		ZONGDAN_digestion_temp=f.f;
		acqdata_set_value_flag(acq_data,"i13004",UNIT_0C,ZONGDAN_digestion_temp,INFOR_ARGUMENTS,&arg_n);	
		...
		f.ch[3]=com_rbuf[67];
		ZONGDAN_range=f.f;
		acqdata_set_value_flag(acq_data,"i13116",UNIT_MG_M3,ZONGDAN_range,INFOR_ARGUMENTS,&arg_n);	
	...
	{
		ZONGDAN_zero=0;
		ZONGDAN_correction_factor=0;
		ZONGDAN_digestion_time=0;
		ZONGDAN_digestion_temp=0;
		ZONGDAN_range=0;
		status=1;
	}
