...
int protocol_COD_NJruiquan(struct acquisition_data *acq_data)
{
   ...
   sleep(1);
	...
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
   	SYSLOG_DBG("NJruiquan COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG("data:%s\n",com_rbuf);
    
	pf=acq_data->com_f;       
   if(size>=9 && com_rbuf[0]==0x55 && com_rbuf[1]==0x55
	 	&& com_rbuf[size-1]==0x55 && com_rbuf[size-2]==0x55)
   {               
		pf[arg_n] = *((float *)(&com_rbuf[5]));
