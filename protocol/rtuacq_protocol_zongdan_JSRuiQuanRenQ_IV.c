...
Description:protocol_ZONGDAN_JSRuiQuanRenQ_IV
TX:01 03 00 00 00 02 C4 0B  
RX:01 03 04 32 33 41 03 74 d5
...
int protocol_ZONGDAN_JSRuiQuanRenQ_IV(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV ZONGDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV ZONGDAN protocol,ZONGDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV ZONGDAN data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV ZONGDAN RECV:",com_rbuf,size);
	...
	acqdata_set_value(acq_data,"w21001",UNIT_MG_L,valf,&arg_n);
	if(status == 0)
...
int protocol_ZONGDAN_JSRuiQuanRenQ_IV_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGDAN_JSRuiQuanRenQ_IV_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV TN INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV TN INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV TN INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV TN INFO RECV:",com_rbuf,size);
	...
	char* p = NULL;
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf, devaddr, 0x03, 0x6C, 0x02);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"JSRuiQuanRenQ_IV TN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data), com_tbuf, size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf, sizeof(com_rbuf)-1);
	SYSLOG_DBG("JSRuiQuanRenQ_IV TN protocol,TN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("JSRuiQuanRenQ_IV TN data ",com_rbuf,size);	
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"JSRuiQuanRenQ_IV TN RECV:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, 0x03, 0x02);
	if(p!= NULL)
	{
		valf = getFloatValue(p, 3, FLOAT_CDAB);
