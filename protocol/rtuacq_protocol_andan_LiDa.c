...
static float ANDAN_LiDa_orig=0;

/*
Author:Yimning
Time:2021.06.09 Wed.
Description:protocol_ANDAN_LiDa
...
int protocol_ANDAN_LiDa(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"LiDa ANDAN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("LiDa ANDAN protocol,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("LiDa ANDAN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"LiDa ANDAN RECV:",com_rbuf,size);
	...
	ret=acqdata_set_value_orig(acq_data,"w21003",UNIT_MG_L,total_ANDAN,ANDAN_LiDa_orig,&arg_n);
	if(status == 0)
...
Author:Yimning
Time:2021.06.09 Wed.
Description:protocol_ANDAN_LiDa_info
...
*/	
int protocol_ANDAN_LiDa_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	float ANDAN_zero=0,ANDAN_range=0,ANDAN_K=0,ANDAN_PPB=0;
	float ANDAN_correction_factor=0,ANDAN_adjust_zero_drift=0,ANDAN_range_drift=0,ANDAN_intercept=0,ANDAN_BDXS=0,cod_original_Val=0;
	float ANDAN_digestion_time=0,ANDAN_digestion_temp=0;
	...
	SYSLOG_DBG("protocol_ANDAN_LiDa_info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,0x3,0x06E8,0x23);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol_ANDAN_LiDa_info SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);

	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol_ANDAN_LiDa_info,ANDAN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol_ANDAN_LiDa_info data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol_ANDAN_LiDa_info RECV:",com_rbuf,size);
	if((size>=75)&&(com_rbuf[0]==devaddr)&&(com_rbuf[1]==0x03))
	{
		...
		f.ch[2]=com_rbuf[32];
		f.ch[3]=com_rbuf[31];
		ANDAN_zero=f.f;   //ANDAN_zero_val
    	//acqdata_set_value_flag(acq_data,"i13104",UNIT_L,ANDAN_zero,INFOR_ARGUMENTS,&arg_n);

    	//acqdata_set_value_flag(acq_data,"i13105",UNIT_NONE,ANDAN_LiDa_orig,INFOR_ARGUMENTS,&arg_n);
	...
	//acqdata_set_value_flag(acq_data,"i13003",UNIT_MINUTE,val,INFOR_ARGUMENTS,&arg_n);
		...
		f.ch[2]=com_rbuf[54];
		f.ch[3]=com_rbuf[53];
		ANDAN_correction_factor=f.f;   //ANDAN_correction_factor
    	acqdata_set_value_flag(acq_data,"i13119",UNIT_NONE,ANDAN_correction_factor,INFOR_ARGUMENTS,&arg_n);

		f.ch[0]=com_rbuf[56];
		f.ch[1]=com_rbuf[55];
		f.ch[2]=com_rbuf[58];
		f.ch[3]=com_rbuf[57];
		ANDAN_adjust_zero_drift=f.f;   //ANDAN_adjust_zero_drift
    	acqdata_set_value_flag(acq_data,"i13120",UNIT_NONE,ANDAN_adjust_zero_drift,INFOR_ARGUMENTS,&arg_n);	

	sprintf(hexarry,"%x%x",com_rbuf[59],com_rbuf[60]);
	ANDAN_digestion_time = atoi(hexarry);
	acqdata_set_value_flag(acq_data,"i13005",UNIT_MINUTE,ANDAN_digestion_time,INFOR_ARGUMENTS,&arg_n);	
		...
		f.ch[2]=com_rbuf[64];
		f.ch[3]=com_rbuf[63];
		ANDAN_digestion_temp=f.f;
		acqdata_set_value_flag(acq_data,"i13004",UNIT_0C,ANDAN_digestion_temp,INFOR_ARGUMENTS,&arg_n);	
		...
		f.ch[3]=com_rbuf[67];
		ANDAN_range=f.f;
		acqdata_set_value_flag(acq_data,"i13116",UNIT_MG_M3,ANDAN_range,INFOR_ARGUMENTS,&arg_n);	
	...
	{
		ANDAN_zero=0;
		ANDAN_correction_factor=0;
		ANDAN_digestion_time=0;
		ANDAN_digestion_temp=0;
		ANDAN_range=0;
		status=1;
	}
