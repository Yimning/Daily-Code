...
int protocol_PH_221B(struct acquisition_data *acq_data)
{
	...
	size=getHexDataPack(DEV_NAME(acq_data), com_tbuf, size, com_rbuf, sizeof(com_rbuf), "PH221B ", 1);
	p=modbus_crc_check(com_rbuf, size, devaddr, cmd, cnt);
	if(p!=NULL)
	{
		ph=getUInt16Value(p, 3, dataType);
		val=getUInt16Value(p, 5, dataType);
		for(i=0;i<val;i++)
			ph*=0.1;

		temp=getUInt16Value(p, 11, dataType);
		val=getUInt16Value(p, 13, dataType);
		for(i=0;i<val;i++)
			temp*=0.1;
