...
Description:protocol_ZONGLIN_NJJuGe_New
...
static float zonglin_orig = 0;
static int TP_NJJuGe_MARK = 0;
int protocol_ZONGLIN_NJJuGe_New(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe TP SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe TP protocol,TP : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe TP data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe TP RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		zonglin = getFloatValue(p, 5, dataType);
		
		zonglin_orig= getFloatValue(p, 9, dataType);

		//val = getInt16Value(p, 3, INT_AB);
		
		switch(TP_NJJuGe_MARK)
		{
	...
	ret=acqdata_set_value_orig(acq_data,"w21011",UNIT_MG_L,zonglin,zonglin_orig,&arg_n);
...
int protocol_ZONGLIN_NJJuGe_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ZONGLIN_NJJuGe_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe TP INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe TP INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe TP INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe TP INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
 		val = getInt16Value(p, 3, INT_AB);
		...
		TP_NJJuGe_MARK = getInt16Value(p, 11, INT_AB);
