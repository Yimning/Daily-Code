...
Description:protocol_COD_NJJuGe_New
...
static float cod_orig = 0;
static int COD_NJJuGe_MARK = 0;
int protocol_COD_NJJuGe_New(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe COD data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe COD RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		cod = getFloatValue(p, 5, dataType);
		
		cod_orig= getFloatValue(p, 9, dataType);

		val = getInt16Value(p, 3, INT_AB);
		
		switch(COD_NJJuGe_MARK)
		{
	...
	ret=acqdata_set_value_orig(acq_data,"w01018",UNIT_MG_L,cod,cod_orig,&arg_n);
...
int protocol_COD_NJJuGe_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe COD INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe COD INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe COD INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe COD INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
 		val = getInt16Value(p, 3, INT_AB);
		...
		COD_NJJuGe_MARK = getInt16Value(p, 11, INT_AB);
