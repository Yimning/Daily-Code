...
int protocol_COD_GLYunJing_HeBei(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei COD SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(p,0,sizeof(p));
	size=read_device(DEV_NAME(acq_data),p,sizeof(p)-1);
	SYSLOG_DBG("GLYunJing HeBei COD protocol,COD : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei COD data",p,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei COD RECV:",p,size);
	com_rbuf=modbus_crc_check(p, size, devaddr, 0x03, 0x02);
	if(com_rbuf!=NULL)
	{
		f.ch[0]=com_rbuf[4];
		f.ch[1]=com_rbuf[3];
	...
	else
	{
		...
		LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei COD SEND again:",com_tbuf,size);
		size=write_device(DEV_NAME(acq_data),com_tbuf,size);
		sleep(1);
		memset(p,0,sizeof(p));
		size=read_device(DEV_NAME(acq_data),p,sizeof(p)-1);
		SYSLOG_DBG("GLYunJing HeBei COD protocol,COD : read again device %s , size=%d\n",DEV_NAME(acq_data),size);
		SYSLOG_DBG_HEX("GLYunJing HeBei COD data",p,size);
		LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei COD RECV again:",p,size);
		com_rbuf=modbus_crc_check(p, size, devaddr, 0x03, 0x02);
		if(com_rbuf!=NULL)
		{
			f.ch[0]=com_rbuf[4];
...
static float  COD_GLYunJing_HeBei_Stay = 0;
	int status=0;
	float *pf;
	union uf  f;
	char com_rbuf[2048]={0}; 
	...
	if(p!=NULL)
	{
		...
		switch(val)
		{
			case 0:	flag='N';COD_GLYunJing_HeBei_Stay = valf;break;
			//case 1:	flag='T';break;
			//case 2:	flag='T';break;
			case 3:	flag='D';break;
			case 4:	flag='M';break;
...
int protocol_COD_GLYunJing_HeBei_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_COD_GLYunJing_HeBei_info\n");
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei COD STATUS SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei COD STATUS protocol,STATUS: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei COD STATUS data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei COD STATUS RECV:",com_rbuf,size);
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"GLYunJing HeBei COD INFO SEND1:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("GLYunJing HeBei COD INFO protocol,INFO 1: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("GLYunJing HeBei COD INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"GLYunJing HeBei COD INFO RECV1:",com_rbuf,size);
