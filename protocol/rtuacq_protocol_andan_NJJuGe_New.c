...
Description:protocol_ANDAN_NJJuGe_New
...
static float nh3n_orig = 0;
static  int N3HN_NJJuGe_MARK = 0;
int protocol_ANDAN_NJJuGe_New(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe NH3N SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe NH3N protocol,NH3N : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe NH3N data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe NH3N RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
		nh3n = getFloatValue(p, 5, dataType);
		
		nh3n_orig= getFloatValue(p, 9, dataType);

		val = getInt16Value(p, 3, INT_AB);
		
		switch(N3HN_NJJuGe_MARK)
		{
	...
	ret=acqdata_set_value_orig(acq_data,"w21003",UNIT_MG_L,nh3n,nh3n_orig,&arg_n);
...
int protocol_ANDAN_NJJuGe_New_info(struct acquisition_data *acq_data)
{
	int status=0;
	char com_rbuf[2048]={0}; 
	char com_tbuf[8]={0};
	...
	if(!acq_data) return -1;
	SYSLOG_DBG("protocol_ANDAN_NJJuGe_info\n");
	...
	memset(com_tbuf,0,sizeof(com_tbuf));
	size=modbus_pack(com_tbuf,devaddr,cmd,regpos,regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"NJJuGe NH3N INFO SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("NJJuGe NH3N INFO protocol,INFO: read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("NJJuGe NH3N INFO data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"NJJuGe NH3N INFO RECV:",com_rbuf,size);
	p=modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!=NULL)
	{
 		val = getInt16Value(p, 3, INT_AB);
		...
		N3HN_NJJuGe_MARK  = getInt16Value(p, 11, INT_AB);
		status = 0;
	}
