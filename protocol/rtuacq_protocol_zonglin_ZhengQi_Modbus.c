...
Description:protocol_ZONGLIN_ZhengQi_Modbus
...
int protocol_ZONGLIN_ZhengQi_Modbus(struct acquisition_data *acq_data)
{
	...
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"ZhengQi_Modbus ZONGLIN SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("ZhengQi_Modbus ZONGLIN protocol,ZONGLIN : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("ZhengQi_Modbus ZONGLIN data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"ZhengQi_Modbus ZONGLIN RECV:",com_rbuf,size);
	...
	ret=acqdata_set_value_orig(acq_data,"w21011",UNIT_MG_L,valf,org,&arg_n);
...
Author:Yimning
Create Time:2021.08.31 Tues.
Description:protocol_ZONGLIN_ZhengQi_Modbus_info
...
*/
int protocol_ZONGLIN_ZhengQi_Modbus_info(struct acquisition_data *acq_data)
{
	int status=0;
	float *pf;
	union uf  f;
	...
	regcnt = 0x43;
	dataType = FLOAT_ABCD ;
	SYSLOG_DBG("protocol ZONGLIN ZhengQi_Modbus info\n");
	memset(com_tbuf,0,sizeof(com_tbuf));
	acqdata_set_value_flag(acq_data,"w21011",UNIT_MG_M3,valf,INFOR_ARGUMENTS,&arg_n);
	size=modbus_pack(com_tbuf, devaddr, cmd, regpos, regcnt);
	LOG_WRITE_HEX(DEV_NAME(acq_data),0,"protocol ZONGLIN ZhengQi_Modbus info SEND:",com_tbuf,size);
	size=write_device(DEV_NAME(acq_data),com_tbuf,size);
	sleep(1);
	memset(com_rbuf,0,sizeof(com_rbuf));
	size=read_device(DEV_NAME(acq_data),com_rbuf,sizeof(com_rbuf)-1);
	SYSLOG_DBG("protocol ZONGLIN ZhengQi_Modbus, Info : read device %s , size=%d\n",DEV_NAME(acq_data),size);
	SYSLOG_DBG_HEX("protocol ZONGLIN ZhengQi_Modbus Info data",com_rbuf,size);
	LOG_WRITE_HEX(DEV_NAME(acq_data),1,"protocol ZONGLIN ZhengQi_Modbus info RECV:",com_rbuf,size);
	p = modbus_crc_check(com_rbuf,size, devaddr, cmd, regcnt);
	if(p!= NULL)
	{
